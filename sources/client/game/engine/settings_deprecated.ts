...
//GAME
let device_info = Device.getInfo();
export var sound_effects = device_info.is_mobile ? 1.0 : 0.5;//0 -> 1
export var game_panel_auto_hide = true;
export var weather_particles = true;
export var canvas_rendering = false;

export var painter_resolution = device_info.is_mobile ? 'MEDIUM' : 'HIGH';//'LOW', 'MEDIUM', 'HIGH'
export var shadows_type = 'LONG';//'LONG', 'FLAT;

//MENU
export var menu_background_effect = false;
export var menu_click_effect = !device_info.is_mobile;//DISABLE FOR MOBILE

//CHAT
export var chat_auto_hide_show = !device_info.is_mobile;//DISABLE FOR MOBILE
...
function cast(value: any, type: string) {
	if(value === undefined || value === null) 
		throw new Error('Given value must be defined');
	switch(type) {
		default:
		case 'string':
			return String(value);
