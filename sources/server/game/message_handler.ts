...
async function loginAsUser(connection: Connection, token: string) {
  ...
  if (res.error !== ERROR_CODES.SUCCESS || !res.account)
    ...
    if (await findDuplicateSession(res.account.id)) {
      ...
      connection.loginAsUser(
        res.account.id,
        token,
        AccountSchema2UserCustomData(res.account),
      )
  }
}
...
export async function handleJSON(connection: Connection, data: NetworkPackage) {
  ...
  switch (data.type) {
    default:
      ...
      break
    case NetworkCodes.LOGIN:
      {
        //data.token: string | null
        if (typeof data.token === 'string')
          await loginAsUser(connection, data.token)
        //LOGIN AS GUEST
        else connection.loginAsGuest()
      }
      break

    case NetworkCodes.ACCOUNT_DATA_REQUEST: //authenticating via session token
      if (!connection.user) break
      if (!connection.user.isGuest() && connection.user.account_id) {
        //user logged out or session expired
        if (
          typeof data.token !== 'string' ||
          connection.user.session_token !== data.token
        ) {
          if (connection.getRoom()) RoomManager.leaveRoom(connection)
          connection.loginAsGuest()
        } else {
          let account_schema = await Database.getAccount(
            connection.user.account_id,
          )
          if (!account_schema)
            throw new Error(
        ...
        if (typeof data.token === 'string') {
          //user just logged in
          if (connection.getRoom()) RoomManager.leaveRoom(connection)
          loginAsUser(connection, data.token).catch((e) =>
            console.error('Cannot login as user:', e),
          )
        } else connection.updateUserData(null)
      }
      break
    ...
    case NetworkCodes.JOIN_ROOM_REQUEST:
      if (typeof data.id === 'number') RoomManager.joinRoom(connection, data.id)
      break
    ...
    case NetworkCodes.ROOM_SETTINGS_UPDATE_REQUEST:
      if (
        ...
        typeof data.sits_number === 'number' &&
        typeof data.duration === 'number' &&
        typeof data.max_enemies === 'number'
      ) {
    ...
    case NetworkCodes.START_GAME_CONFIRMATION:
      {
        ...
        if (game) game.onConfirmation(connection)
      }
      break
  }
}
