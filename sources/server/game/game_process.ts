...
console.log = (function (MSG_PREFIX) {
  ...
  return function () {
    log.apply(console, [MSG_PREFIX, ...arguments])
  }
})(`[Process:${process.pid}] =>`)
...
let waitForGameInitialize = (game: ServerGame, callback: () => void) => {
  if (game.initialized) callback()
  else setTimeout(() => waitForGameInitialize(game, callback), 100)
}
      ...
      case PROCESS_ACTIONS.INIT_GAME:
        {
          ...
          msg.playing_users.forEach((user) =>
            room.addUser(UserInfo.fromFullJSON(user)),
          )

          onMapsLoaded(() => {
            //make sure maps data is loaded
            try {
              if (!(room.map in Maps)) {
              ...
              if (games.has(room.id)) {
                console.error(
                  'Game with given id is already initialized: ' + room.id,
                )
                return
              }
      ...
      case PROCESS_ACTIONS.START_GAME:
        {
          ...
          if (!game_to_start) {
            console.error(
              'Cannot find game with given id to run: ' + msg.room_id,
            )
            // @ts-ignore
            process.send({ action: NetworkCodes.START_GAME_FAIL_ACTION })
            return
          ...
          waitForGameInitialize(game_to_start, () => {
            if (game_to_start) game_to_start.start()
      ...
      case PROCESS_ACTIONS.ON_EVERYONE_LEFT:
        {
          ...
          if (game_to_finish) game_to_finish.finishImmediately()
        }
        break
    }
  }
