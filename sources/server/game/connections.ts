...
export class Connection {
  ...
  private send<T>(data: T & Exclude<T, string>) {
    //stringified json
  ...
  public sendCustom<T>(
    data: { [index: string]: T & Exclude<T, string> } & { type: NetworkCodes },
  ) {
    this.send(data)
    if (
      data.type === NetworkCodes.END_GAME &&
      this.user &&
  ...
  private static updateSocialRoomData(
    account_id: string,
  ...
  private static updateSocialIsPlayingState(
    account_id: string,
    is_playing: boolean,
  ) {
    let social_connections = SocialConnection.getConnections(account_id)
  ...
  updateUserData(fresh_data: UserCustomData | null) {
    if (!this.user) throw new Error('This connection has no user')

    if (fresh_data) this.user.updateData(fresh_data)
  ...
  onRoomCreated(room: RoomInfo) {
    //send data for user's rooms list
    if (!this.user) throw new Error('This connection has no user')
  ...
  onRoomJoined() {
    //sends user's current room data
    if (!this.user) throw new Error('This connection has no user')
    if (!this.user.room) throw new Error('User has no room assigned')
...
export default {
  ...
  remove(connection: Connection) {
    if (connection.user) connection.user.connection = null
    if (!connections.delete(connection.id))
      console.error(
        'Cannot delete connection. Object not found in map structure.',
      )
  },

  findAccount(account_id: string) {
    for (let [, conn] of connections) {
      if (conn.user && conn.user.account_id === account_id) return conn.user
    }
    return undefined
  },
  ...
  forEachLobbyUser(func: (connection: Connection) => void) {
    connections.forEach((connection) => {
      if (connection.isInLobby()) func(connection)
    })
  },

  forEachAccountUser(func: (connection: Connection) => void) {
    //each logged in user
    connections.forEach((connection) => {
      if (connection.user && connection.user.account_id) func(connection)
    })
  },
}
