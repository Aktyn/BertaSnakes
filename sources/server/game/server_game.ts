...
const runLoop = (function () {
  ...
  return function (self: ServerGame) {
    function loop() {
      ...
      _steps_ = 0
      while (delta_sum >= FIXED_DELTA && _steps_ < 32) {
        //maximum 32 iterations per frame
...
export default class ServerGame extends GameCore implements CollisionsDetector {
  ...
  constructor(map: MapJSON_I, room: RoomInfo) {
    ...
    this.maximum_enemies = room.max_enemies //MAXIMUM_ENEMIES;
    ...
    try {
      ...
      if (!result) throw new Error('Cannot load map')
    } catch (e) {
      console.error(e)
  ...
  public finishImmediately() {
    console.log(
      `No one is watching this game anymore, finishing it now (${this.room.id})`,
    )
  ...
  getPlayerByUserId(id: number) {
    for (async_p_i = 0; async_p_i < this.players.length; async_p_i++) {
      //@ts-ignore
      if (this.players[async_p_i].user_id === id) return async_p_i //this.players[async_p_i];
    }
    return -1
  }

  onClientMessage(client_id: number, data: Uint8Array) {
    if ((async_p_i = this.getPlayerByUserId(client_id)) === -1) return
    async_p_it = this.players[async_p_i]

    if (async_p_it.spawning) return
    ...
    switch (data[0]) {
      case NetworkCodes.PLAYER_MOVEMENT:
        ...
        this.emitAction(NetworkCodes.SEND_DATA_TO_CLIENT_ACTION_FLOAT32, [
          NetworkCodes.PLAYER_MOVEMENT_UPDATE,
          async_p_i,
          async_p_it.rot,
          data[1],
          async_p_it.movement.speed,
        ])

        break
      case NetworkCodes.PLAYER_EMOTICON:
        this.dataForClients.push(
          NetworkCodes.ON_PLAYER_EMOTICON,
          async_p_i,
          data[1],
        )
        break
      ...
      case NetworkCodes.PLAYER_SKILL_STOP_REQUEST:
        ...
        if (async_s) {
          async_s.stopUsing()
          this.emitAction(NetworkCodes.SEND_DATA_TO_CLIENT_ACTION_FLOAT32, [
            NetworkCodes.ON_PLAYER_SKILL_CANCEL,
            async_p_i,
            data[1],
          ])
        }
  ...
  onPlayerPainterCollision(player: Player, color: Uint8Array) {
    ...
    if (Colors.compareByteBuffers(this.walls_color.byte_buffer, color)) {
      if (player.spawning === true) {
        //pushing out of safe area and finishing spawning
        this.bounceVec
          .set(player.x, player.y)
          .normalize()
          .scaleBy(
            GameCore.GET_PARAMS().spawn_radius +
              GameCore.GET_PARAMS().spawn_walls_thickness,
          )
        ...
        this.dataForClients.push(
          NetworkCodes.ON_PLAYER_SPAWNING_FINISH,
          this.players.indexOf(player),
          player.x,
          player.y,
        )
      } else this.playerBounce(player, color)
      ...
      if (player.effects.isActive(AVAILABLE_EFFECTS.POISONING) === false) {
        ...
        this.dataForClients.push(
          NetworkCodes.ON_PLAYER_POISONED,
          this.players.indexOf(player),
        )
      }
    } else if (Colors.isPlayerColor(color)) {
      //checking collisions with other players curves
  ...
  onBulletPainterCollision(bullet: Bullet, color: Uint8Array) {
    ...
    if (Colors.compareByteBuffers(bullet.color.byte_buffer, color)) return

    if (
      Colors.compareByteBuffers(this.walls_color.byte_buffer, color) ||
      Colors.compareByteBuffers(Colors.POISON.byte_buffer, color) ||
      Colors.isPlayerColor(color)
    ) {
      if (bullet.type === BULLET_TYPE.BOUNCING) {
        //bounce bullet if it is a bouncing type
        ...
        this.dataForClients.push(
          NetworkCodes.ON_BULLET_EXPLODE,
          bullet.id,
          hit_x,
          hit_y,
        )
  ...
  onPlayerEnemyCollision(player: Player, enemy: Enemy) {
    ...
    if (
      player.effects.isActive(AVAILABLE_EFFECTS.SHIELD) === false &&
    ...
    this.dataForClients.push(
      NetworkCodes.ON_PLAYER_ENEMY_COLLISION,
      enemy.id,
      this.players.indexOf(player),
      player.x,
      player.y,
    ...
    if (!player.isAlive()) this.onPlayerDeath(player, 0)
  }
  ...
  onBulletHit(object: Object2D, bullet: Bullet, is_player: boolean) {
    ...
    if (is_player) {
      if (bullet.type === BULLET_TYPE.BOUNCING)
        damage =
          GameCore.GET_PARAMS().player_to_bouncing_bullet_receptivity *
          dmg_scale
      else
        damage = GameCore.GET_PARAMS().player_to_bullet_receptivity * dmg_scale

      this.onPlayerAttackedPlayer(<Player>bullet.parent, <Player>object, damage)
    } else {
      if (bullet.type === BULLET_TYPE.BOUNCING)
        damage =
          GameCore.GET_PARAMS().enemy_to_bouncing_bullet_receptivity * dmg_scale
      else
        damage = GameCore.GET_PARAMS().enemy_to_bullet_receptivity * dmg_scale

      this.onPlayerAttackedEnemy(<Player>bullet.parent, <Enemy>object, damage)
    }

    this.dataForClients.push(
      NetworkCodes.ON_BULLET_HIT,
      bullet.id,
      hit_x,
      hit_y,
    )
  ...
  onPlayerBulletCollision(player: Player, bullet: Bullet) {
    if (!player.isAlive() || bullet.parent === player) return
  ...
  onEnemyBulletCollision(enemy: Enemy, bullet: Bullet) {
    if (!enemy.isAlive())
      //|| bullet.parent === enemy => TODO when enemy will shoot
      return
  ...
  onPlayerItemCollision(player: Player, item: Item) {
    ...
    switch (item.type) {
      ...
      case ITEM_TYPES.HEALTH:
        if (player.hp >= 0.995) return
        player.hp += Item.HEALTH_VALUE
        break
      case ITEM_TYPES.ENERGY:
        if (player.energy >= 0.995) return
    ...
    this.dataForClients.push(
      NetworkCodes.ON_PLAYER_COLLECT_ITEM,
      item.id,
      item.type,
      this.players.indexOf(player),
    )
  ...
  onPlayerEnemyPainterCollision(player: Player) {
    if (
      ...
      player.points -=
        GameCore.GET_PARAMS().points_lose_for_enemy_painter_collision
      player.hp -= GameCore.GET_PARAMS().enemy_painter_collision_damage
      if (player.hp < 0.01) player.hp = 0.01
    }
    player.movement.speed = player.movement.maxSpeed

    this.dataForClients.push(
      NetworkCodes.ON_PLAYER_ENEMY_PAINTER_COLLISION,
      this.players.indexOf(player),
      player.x,
      player.y,
      player.hp,
  ...
  onPlayerAttackedPlayer(attacker: Player, victim: Player, damage: number) {
    if (this.room.gamemode !== GAME_MODES.COMPETITION) return

    victim.hp -= damage //must be before putting data for clients

    this.dataForClients.push(
      NetworkCodes.ON_PLAYER_ATTACKED,
      this.players.indexOf(attacker),
      damage,
      this.players.indexOf(victim),
      victim.hp,
      victim.x,
      victim.y,
    )
  ...
  onPlayerAttackedEnemy(player: Player, enemy: Enemy, damage: number) {
  ...
  onBombExplosion(bomb: Bomb) {
    ...
    for (e_i = 0; e_i < this.enemies.length; e_i++) {
      if (
        !(<Enemy>this.enemies[e_i]).spawning &&
        Vector.distanceSqrt(this.enemies[e_i], bomb) <= radius_pow
      ) {
        //NOTE - 1.0 == 100% damage
        this.onPlayerAttackedEnemy(bomb.parent, <Enemy>this.enemies[e_i], 1.0)
      }
    }
    if (this.room.gamemode === GAME_MODES.COMPETITION) {
      for (p_i = 0; p_i < this.players.length; p_i++) {
        //@ts-ignore
        if (
          this.players[p_i] !== bomb.parent &&
          !this.players[p_i].spawning &&
  ...
  onStain(enemy: Enemy) {
    ...
    this.dataForClients.push(
      NetworkCodes.ON_POISON_STAIN,
      stain_index,
      enemy.x,
      enemy.y,
      enemy.width * GameCore.GET_PARAMS().stain_shrink,
    )
  }
  ...
  initWave() {
    ...
    if (this.enemies.length < this.maximum_enemies) {
      ...
      for (
        wave_i = 0;
        wave_i < ENEMIES_PER_WAVE && this.enemies.length < this.maximum_enemies;
        wave_i++
      ) {
  ...
  applySkillEffect(
    ...
    switch (skill.data) {
      default:
        throw new Error('Incorrect skill type')
      ...
      case Skills.BOUNCE_SHOT:
        {
          ...
          this.dataForClients.push(
            NetworkCodes.ON_BOUNCE_BULLET_SHOT,
            player_i,
            bullet.id,
            bullet.x,
            bullet.y,
    ...
    if (!skill.isContinuous()) skill.stopUsing()

    if (immediately_response) {
      this.emitAction(NetworkCodes.SEND_DATA_TO_CLIENT_ACTION_FLOAT32, [
        NetworkCodes.ON_PLAYER_SKILL_USE,
        player_i,
        skill_i,
        player.energy,
      ])
  ...
  updateSpawnings(delta: number) {
    ...
    for (r_p_i = 0; r_p_i < this.respawning_players.length; r_p_i++) {
      if ((this.respawning_players[r_p_i].time -= delta) <= 0) {
        //spawning finished
        ...
        this.dataForClients.push(
          NetworkCodes.ON_IMMUNITY_EFFECT,
          this.players.indexOf(p_it),
        )
  ...
  updateChunks() {
    for (chunk_it = 0; chunk_it < this.chunks.length; chunk_it++) {
      ...
      if (chunk_ref.need_update) {
        ...
        chunk_ref.buff = chunk_ref.ctx.getImageData(
          0,
          0,
          chunk_ref.canvas.width,
          chunk_ref.canvas.height,
        )
      }
    }
  }
  ...
  update(delta: number) {
    //NOTE - server side delta is fixed
    delta /= 1000.0
    ...
    for (ss_i = 0; ss_i < this.server_synchronized.length; ss_i++) {
      ...
      for (obj_i = 0; obj_i < synch_array.length; obj_i++) {
        if (
          ++synch_array[obj_i].frames_since_last_update > SYNC_EVERY_N_FRAMES
        ) {
          synch_array[obj_i].frames_since_last_update = 0

          this.dataForClients.push(
            NetworkCodes.OBJECT_SYNCHRONIZE,
            synch_array[obj_i].id,
            ss_i,
            synch_array[obj_i].x,
            synch_array[obj_i].y,
    ...
    for (p_i = 0; p_i < this.players.length; p_i++) {
      ...
      //handling skills use
      for (s_i = 0; s_i < p_it.skills.length; s_i++) {
        //for each player skills
        s_h = p_it.skills[s_i]
        if (s_h === null) continue
        if (s_h.isInUse()) {
      ...
      if (p_it.painter.active && segmentLength > p_it.width / 2.0) {
        ...
        super.drawLine(
          p_it.x,
          p_it.y,
          p_it.painter.lastPos.x,
          p_it.painter.lastPos.y,
    ...
    if (this.dataForClients.length > 0) {
      //if something was added to array
      this.emitAction(
        NetworkCodes.SEND_DATA_TO_CLIENT_ACTION_FLOAT32,
        this.dataForClients,
      )
