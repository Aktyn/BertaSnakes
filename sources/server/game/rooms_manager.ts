...
function distributeRoomRemoveEvent(room: RoomInfo) {
  //distribute room remove event for everyone in lobby
...
setInterval(() => {
  //making sure there are at least few rooms existing
...
export default {
  ...
  createRoom(from_connection: Connection) {
    if (!from_connection.user) return
  ...
  deleteRoomAfterGame(room: RoomInfo) {
    ...
    if (!rooms.delete(room.id)) console.error('Cannot delete room: ' + room.id)
  },

  restoreRoomAfterGame(room: RoomInfo) {
    //makes everyone in lobby see this room on rooms list
    distributeRoomCreateEvent(room)
  },

  updateRoomSettings(from_connection: Connection, settings: RoomSettings) {
    let room = from_connection.getRoom()
    if (!room || room.getOwner() !== from_connection.user)
  ...
  sendRoomsList(from_connection: Connection) {
    if (from_connection.isInLobby()) {
      let available_rooms = Array.from(rooms.values()).filter((room) => {
        ...
        return (
          !room.game_handler &&
          from_connection.user &&
          !room.banned_ips.has(from_connection.client_ip)
        )
      })
  ...
  leaveRoom(from_connection: Connection) {
    ...
    if (room.isEmpty()) {
      if (!room.game_handler) {
        ...
        //IF IT IS DURING GAME THEN SPEED IT UP
        ;(<GameHandler>room.game_handler).onRoomEmptied(room)
      }
    } else {
      //send USER_LEFT_ROOM for every other user in room
  ...
  joinRoom(from_connection: Connection, room_id: number) {
    ...
    if (current_room) this.leaveRoom(from_connection)
    ...
    if (room.game_handler) {
      from_connection.sendNotification(
        NotificationCodes.CANNOT_JOIN_AFTER_GAME_START,
      )
      return
    }
  ...
  kickUser(from_connection: Connection, user_id: number) {
    ...
    if (!room || !from_user || from_user.id === user_id)
      //user cannot kick himself
      return

    if (room.game_handler && room.isUserSitting(user_id))
      //cannot kick player during game
      return

    let room_owner = room.getOwner()
    if (room_owner && from_user.id === room_owner.id) {
      //user is room owner so he can kick
      let user_to_kick = room.getUserByID(user_id)
      if (!user_to_kick || !user_to_kick.connection)
        //no user to kick or is he not connected
        return
  ...
  sitUser(from_connection: Connection) {
    let room = from_connection.getRoom()
    if (!room || !from_connection.user) return
  ...
  readyUser(from_connection: Connection) {
    let room = from_connection.getRoom()
    if (!room || !from_connection.user) return
    if (room.everyoneReady() || !room.everyoneSits()) return
    ...
    if (room.everyoneReady()) {
      try {
        ...
        //countdown failure - unready everyone
        room.unreadyAll()
        distributeRoomUpdateEvent(room)
      }
    }
  ...
  sendRoomMessage(from_connection: Connection, msg: string) {
    ...
    room.forEachUser((user) => {
      ...
      user.connection.sendRoomMessage(
        (<RoomInfo>room).id,
        message.author_id,
        message.timestamp,
        message.content,
    ...
    room.forEachUser((user) => {
      if (user.connection) {
        user.connection.sendGameStartFailEvent(
          room,
          unconfirmed_users
            ? ReasonCodes.USER_DOES_NOT_CONFIRM_GAME
