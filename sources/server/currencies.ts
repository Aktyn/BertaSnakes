...
type CurrenciesDataResponse =
  | CurrenciesDataErrorResponse
  | CurrenciesDataSuccessResponse
...
export default async function getCurrenciesData(): Promise<CurrenciesDataResponse> {
  ...
  if (cached_currencies) return cached_currencies.data

  //load from openexchangerates.org api; https://openexchangerates.org/account/usage
  let api_res: OpenexchangeApiRes = (process.env.NODE_ENV || '').match(
    /developement|dev/i,
  )
    ? dev_api_res
    : await fetch(
        `https://openexchangerates.org/api/latest.json?app_id=${OPENEXCHANGE_APP_ID}`,
