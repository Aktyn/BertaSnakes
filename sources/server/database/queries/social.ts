...
function mapFriendshipData(data: any[], is_left: boolean) {
  return data
    .map((friendship_data) => {
      if (friendship_data['friend'].length < 1) return null
      return {
...
export default {
  async getFriendshipID(self_hex: string, friend_hex: string) {
    try {
      ...
      if (found_on_left)
        return {
          id: (found_on_left['_id'] as ObjectId).toHexString(),
          left: true,
        }
      ...
      if (found_on_right)
        return {
          id: (found_on_right['_id'] as ObjectId).toHexString(),
          left: false,
        }
  ...
  async getAccountFriends(account_hex_id: string) {
    try {
      ...
      let from_friends = await getCollection(COLLECTIONS.friendships)
        .aggregate([
          {
            $match: {
              to: account_id,
        ...
        .toArray()
      let to_friends = await getCollection(COLLECTIONS.friendships)
        .aggregate([
          {
            $match: {
              from: account_id,
  ...
  async getAccountFriendRequests(account_hex_id: string) {
    //returns those accounts who requested friendship
    try {
      let account_id = ObjectId.createFromHexString(account_hex_id)

      let requesting_friends = await getCollection(COLLECTIONS.friendships)
        .aggregate([
          {
            $match: {
      ...
      let potential_friends = requesting_friends
        .map((friendship_data) => {
          if (friendship_data['requesting_friend'].length < 1) return null
  ...
  async getAccountRequestedFriends(account_hex_id: string) {
    //returns those friends that user send request to
    try {
      let account_id = ObjectId.createFromHexString(account_hex_id)

      let requested_friends_res = await getCollection(COLLECTIONS.friendships)
        .aggregate([
          {
            $match: {
              accepted: false, //not accepted request
      ...
      let requested_friends = requested_friends_res
        .map((friendship_data) => {
          if (friendship_data['requested_friend'].length < 1) return null
  ...
  async insertAccountFriendRequest(from_hex: string, to_hex: string) {
    try {
      ...
      if (find_res) return { error: ERROR_CODES.FRIENDSHIP_ALREADY_EXISTS }
      ...
      if (!insert_res.result.ok) return { error: ERROR_CODES.DATABASE_ERROR }
  ...
  async acceptFriendship(friend1_id: string, friend2_id: string) {
    try {
      ...
      if (!update_res.result.ok) return { error: ERROR_CODES.DATABASE_ERROR }
      return { error: ERROR_CODES.SUCCESS }
    } catch (e) {
      console.error(e)
      return { error: ERROR_CODES.DATABASE_ERROR }
  ...
  async insertMessage(
    friendship_hex: string,
    is_left_friend: boolean,
    content: string,
  ) {
    try {
      let insert_res = await getCollection(
        COLLECTIONS.social_messages,
      ).insertOne({
  ...
  async loadSocialMessages(friendship_hex: string) {
    try {
      let messages_raw = await getCollection(COLLECTIONS.social_messages)
        .aggregate([
          {
            $match: {
              friendship_id: ObjectId.createFromHexString(friendship_hex),
