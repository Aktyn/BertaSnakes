...
export default {
  async login(_username: string, _password: string) {
    try {
      ...
      if (!account) return { error: ERROR_CODES.USERNAME_NOT_FOUND }
      if (account.password === _password)
        return {
          error: ERROR_CODES.SUCCESS,
          account: extractAccountSchema(account),
        }
      else return { error: ERROR_CODES.INCORRECT_PASSWORD }
    } catch (e) {
      console.error(e)
  ...
  async updateAvatar(account_hex_id: string, _avatar: string | null) {
    try {
      ...
      if (!res.result.ok) return { error: ERROR_CODES.DATABASE_ERROR }
  ...
  async getUserPublicData(account_hex_id: string) {
    try {
      ...
      if (!user) return { error: ERROR_CODES.ACCOUNT_DOES_NOT_EXIST }
  ...
  async getAccountFromToken(_token: string) {
    try {
      ...
      if (
        !session_data ||
        session_data.expiration <= Date.now() ||
      ...
      if (!account) return { error: ERROR_CODES.ACCOUNT_DOES_NOT_EXIST }
      return {
        error: ERROR_CODES.SUCCESS,
        account: extractAccountSchema(account),
      }
  ...
  async getAccountFromEmail(_email: string) {
    try {
      ...
      if (!account) return { error: ERROR_CODES.EMAIL_IS_NOT_REGISTERED }
      return {
        error: ERROR_CODES.SUCCESS,
        account: extractAccountSchema(account),
      }
  ...
  async insertAccount(
    ...
    try {
      ...
      if (existing_nick) return { error: ERROR_CODES.USERNAME_TAKEN }

      let existing_email = await accounts.findOne({
        email: _email,
      })
      if (existing_email) return { error: ERROR_CODES.EMAIL_ALREADY_IN_USE }
      ...
      return {
        error: ERROR_CODES.SUCCESS,
        inserted_id: insert_res.insertedId.toHexString(),
      }
    } catch (e) {
      console.error(e)
      return { error: ERROR_CODES.DATABASE_ERROR }
  ...
  async updateAccountSubscription(
  ...
  async getAccountSubscription(account_hex_id: string) {
    try {
      ...
      return {
        error: ERROR_CODES.SUCCESS,
        subscription: account.subscription as string | undefined | null,
      }
    } catch (e) {
      console.error(e)
      return { error: ERROR_CODES.DATABASE_ERROR }
  ...
  async searchAccount(_username: string) {
    try {
      let accounts = await getCollection(COLLECTIONS.accounts)
        .aggregate([
          {
            $match: {
