...
async function clearBrokenFriendships() {
  //removes social messages that does not belong to any friendship
  let res = (await getCollection(COLLECTIONS.social_messages)
    .aggregate([
      {
        $group: {
          _id: '$friendship_id',
  ...
  if ((delete_res.deletedCount || 0) > 0)
    console.log(
      'Messages from broken friendships removed:',
      delete_res.deletedCount,
    )
}

async function clearOldMessages() {
  //removes social messages older than some threshold
  let res = await getCollection(COLLECTIONS.social_messages).deleteMany({
    _id: {
      $lt: ObjectId.createFromTime(
        (Date.now() - 1000 * 60 * 60 * 24 * 7 * 4) / 1000,
      ), //4 weeks
    },
  })
  if ((res.deletedCount || 0) > 0)
...
async function clearGamesWithoutPlayers() {
  //remove from database game where no account was participating
