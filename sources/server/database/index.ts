...
MongoClient.connect(uri, {
  ...
  .then(async (_client) => {
    ...
    await db
      ...
      .createIndex({ username: 1 }, { name: 'username_search', unique: true })
    await db
      .collection(COLLECTIONS.accounts)
      .createIndex({ email: 'hashed' }, { name: 'email_index' })
    await db
      .collection(COLLECTIONS.accounts)
      .createIndex({ rank: -1 }, { name: 'rank_sorting' })
    await db
      .collection(COLLECTIONS.accounts)
      .createIndex({ creation_time: -1 }, { name: 'creation_date_sorting' })
    await db
      .collection(COLLECTIONS.accounts)
      .createIndex({ level: -1, exp: -1 }, { name: 'level sorting' })

    //sessions
    await db
      .collection(COLLECTIONS.sessions)
      .createIndex({ account_id: 1 }, { name: 'account_session', unique: true })
    await db
      .collection(COLLECTIONS.sessions)
      .createIndex({ token: 'hashed' }, { name: 'session_token' })

    //games
    //not hashed since search is done with use of regexp
    await db
      .collection(COLLECTIONS.games)
      .createIndex({ name: 1 }, { name: 'game_search' })

    //friends
    await db
      .collection(COLLECTIONS.friendships)
      .createIndex({ from: 'hashed' }, { name: 'from-friend-search' })
    await db
      .collection(COLLECTIONS.friendships)
      .createIndex({ to: 'hashed' }, { name: 'to-friend-search' })

    //social messages
    await db
      .collection(COLLECTIONS.social_messages)
      .createIndex({ friendship_id: 'hashed' }, { name: 'friendship_search' })

    //user agents
    await db
      .collection(COLLECTIONS.user_agents)
      .createIndex({ agent: 'hashed' }, { name: 'user_agent_search' })

    //visits
    await db
      .collection(COLLECTIONS.visits)
      .createIndex({ account_id: 'hashed' }, { name: 'filter_by_account' })
    ...
    for (let collection_name of [
      ...
      let size = await db
        ...
        .stats()
        .then((s) => s.storageSize + s.totalIndexSize)
      total_usage += size
    ...
    console.log(
      'Total database size:',
      (total_usage / 1024 / 1024).toFixed(2),
      'MiB',
    )
...
export default {
  onConnect(callback: () => void) {
    if (client) callback()
    else connection_listeners.push(callback)
  },
