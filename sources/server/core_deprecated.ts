...
import UserInfo, {FriendInfoI, UserCustomData} from './../common/user_info';
...
import {PlayerResultJSON, GameResultJSON} from './../common/game/game_result';
...
function createRoom() {
	...
	/*console.log(
		crypto.createHash('md5')
			.update(Date.now().toString()).digest('base64')
	);*/
	...
	switch(json_data.type) {
			...
			if(connection.user.room.everyoneReady() === true) {
				try {
					GameStarter.start_countdown(connection.user.room, 
						Config.START_GAME_COUNTDOWN, startNewGame);
				}
		...
		case NetworkCodes.ROOM_UPDATE_REQUEST: {
			...
			if(_owner !== undefined && _owner.id === connection.user.id) {
				...
				assert(json_data.sits_number <= Config.MAXIMUM_SITS, 
					`number of sits cannot be larger then ${Config.MAXIMUM_SITS}`);
				assert(typeof json_data.duration === 'number', 'duration must be a number');
				assert(json_data.duration > Config.MINIMUM_GAME_DURATION, 
					`duration must be longer or equal ${Config.MINIMUM_GAME_DURATION} minutes`);
				assert(json_data.duration <= Config.MAXIMUM_GAME_DURATION, 
					`duration must be no longer than ${Config.MAXIMUM_GAME_DURATION} minutes`);
				...
				let map = Maps[json_data.map];
				if(map != null)
					room.map = json_data.map;//map.name;
