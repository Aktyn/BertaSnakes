...
function open(app: express.Express) {
  ...
  app.post('/get_visit_statistics', async (req, res) => {
    //token, from, to
    try {
      if (
      ...
      let db_res = await Database.getVisitsStatistics(
        req.body.from,
        req.body.to,
      )
  ...
  app.post('/get_user_visit_statistics', async (req, res) => {
    //token, user_id, from, to
    try {
      if (
        typeof req.body.token !== 'string' ||
        typeof req.body.from !== 'number' ||
      ...
      let db_res = await Database.getUserVisitStatistics(
        req.body.from,
        req.body.to,
        req.body.user_id,
      )
  ...
  app.post('/get_payments_statistics', async (req, res) => {
    //token, from, to
    try {
      if (
        typeof req.body.token !== 'string' ||
      ...
      let db_res = await Database.getPaymentsStatistics(
        req.body.from,
        req.body.to,
      )
      if (db_res.error !== ERROR_CODES.SUCCESS || !db_res.data)
