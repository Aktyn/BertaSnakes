...
async function execBashCmd(req: express.Request, res: express.Response) {
  //token, cmd
  try {
    //works with both: GET and POST request with query strings or JSON body
...
function open(app: express.Express) {
  app.post('/get_version', (req, res) =>
    res.send({ version: global.APP_VERSION }),
  )
  ...
  app.get('/status', async (req, res) => {
    ...
    let usages = (await getMemUsages())
      .map((usage_info, i) => {
        // noinspection SpellCheckingInspection
        return `${i + 1}. games: ${usage_info.games}\tmemory: ${Math.round(
          usage_info.memory / 1024,
        )}MB`
      })
      .join('\n')
