...
import {
  SHIP_COSTS,
  SHIP_LVL_REQUIREMENTS,
} from '../../common/game/objects/player'
...
function open(app: express.Express) {
  app.post('/update_setup', async (req, res) => {
    //token, ship_type, skills
    try {
      if (
        typeof req.body.token !== 'string' ||
        typeof req.body.ship_type !== 'number' ||
      ...
      if (account.available_ships.indexOf(req.body.ship_type) !== -1)
        //requested ship is available
      ...
      for (let skill_id of req.body.skills) {
        ...
        if (
          ...
          account.available_skills.indexOf(skill_id) === -1
        ) {
      ...
      if (are_skills_available) account.skills = req.body.skills

      let update_res = await Database.updateAccountCustomData(
        account.id,
        account,
      )
  ...
  app.post('/buy_ship', async (req, res) => {
    //token, ship_type
    try {
      if (
        typeof req.body.token !== 'string' ||
      ...
      if (type < 0 || type >= SHIP_COSTS.length)
        //basically SHIP_COSTS.length gives number of ship types
        return res.json({ error: ERROR_CODES.INCORRECT_DATA_SENT })
  ...
  app.post('/buy_skill', async (req, res) => {
    //token, skill_id
    try {
      if (
        typeof req.body.token !== 'string' ||
        typeof req.body.skill_id !== 'number'
      )
        return res.json({ error: ERROR_CODES.INCORRECT_DATA_SENT })

      let skill = Skills.getById(req.body.skill_id)
      if (!skill || typeof skill.price !== 'number')
        //given id is not buy-able skill id
        return res.json({ error: ERROR_CODES.INCORRECT_DATA_SENT })
  ...
  app.post('/purchase_coins', async (req, res) => {
    //token, coins, currency
    try {
      if (
        typeof req.body.token !== 'string' ||
        typeof req.body.coins !== 'number' ||
      ...
      const pack_index = packs_array.findIndex(
        (pack) => pack.coins === req.body.coins,
      )
      let pack: CoinPackSchema | undefined
      if (pack_index !== -1) pack = packs_array[pack_index]
      if (!pack) return res.json({ error: ERROR_CODES.INCORRECT_DATA_SENT })
      ...
      if (!origin) return res.json({ error: ERROR_CODES.INCORRECT_DATA_SENT })
      ...
      console.log(
        `purchase request: ${JSON.stringify(pack)}, currency: ${
          req.body.currency
        }, converted price: ${price}, pack name: "${name}", origin: ${origin}`,
      )

      try {
        let payment = await Paypal.createPayment(
          origin,
          name,
          `00${pack_index + 1}`,
          price,
  ...
  app.post('/execute_purchase', async (req, res) => {
    try {
      if (
        typeof req.body.token !== 'string' ||
        typeof req.body.paypal_response !== 'object'
      )
        return res.json({ error: ERROR_CODES.INCORRECT_DATA_SENT })

      const paypal_response = req.body.paypal_response
      if (
        typeof paypal_response.paymentId !== 'string' ||
        typeof paypal_response.PayerID !== 'string' ||
      ...
      if (!payment_data) {
        ...
        } else return res.json({ error: ERROR_CODES.PAYMENT_DATA_NOT_FOUND })
      }
      if (payment_data.account_id !== account.id)
        //only user who created payment can execute it
      ...
      try {
        //add coins to user's account, NOTE: this is done before payment execution
      ...
      try {
        let payment_res = await Paypal.executePayment(
          paypal_response.paymentId,
          paypal_response.PayerID,
        )
        if (payment_res.state !== 'approved')
          console.warn(
            'Payment execution has not been approved!!!',
            payment_res,
          )
      } catch (e) {
        console.error('Payment error: ' + e)
        try {
          //restoring coins state because payment was not successful
