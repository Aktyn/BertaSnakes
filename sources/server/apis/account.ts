...
function open(app: express.Express) {
  ...
  app.post('/register', async (req, res) => {
    ...
    try {
      ...
      if (
        nick < Config.MIN_LOGIN_LENGTH ||
        req.body.password < Config.MIN_PASSWORD_LENGTH
      )
        return res.json({ error: ERROR_CODES.INCORRECT_DATA_SENT })
      const hashed_password = sha256(
        req.body.password.substr(0, Config.MAX_PASSWORD_LENGTH),
      )
      const email = req.body.email

      const verification_code = md5(Date.now().toString() + nick + email)

      let result = await Database.insertAccount(
        nick,
        hashed_password,
        email,
        verification_code,
  ...
  app.post('/request_verification_code', async (req, res) => {
    //token
    try {
      if (typeof req.body.token !== 'string')
        return res.json({ error: ERROR_CODES.INCORRECT_DATA_SENT })
  ...
  app.post('/request_password_reset_code', async (req, res) => {
    //email
    try {
      ...
      reset_codes.forEach((value, key) => {
        if (db_res.account && value === db_res.account.id)
          //found not used reset code for this account
          awaiting_code = key
      })

      if (awaiting_code) {
        //previous code was not used yet
        //resend this code
      ...
      //generate reset code
      let password_reset_code = md5(
        Date.now().toString() + db_res.account.username + req.body.email,
      )
  ...
  app.post('/reset_password', async (req, res) => {
    ...
    try {
      ...
      if (req.body.new_password < Config.MIN_PASSWORD_LENGTH)
        return res.json({ error: ERROR_CODES.INCORRECT_DATA_SENT })
      const hashed_password = sha256(
        req.body.new_password.substr(0, Config.MAX_PASSWORD_LENGTH),
      )
      ...
      let db_res = await Database.updateAccountPassword(
        account_id_to_password_reset,
        hashed_password,
      )
  ...
  app.post('/upload_avatar', async (req, res) => {
    ...
    try {
      ...
      if (req.body.image === null) {
        ...
        let result = await UploadReceiver.saveAvatar(
          avatar_name,
          req.body.image,
        )
        if (result.error !== ERROR_CODES.SUCCESS)
  ...
  app.post('/update_account_subscription', async (req, res) => {
    //token, subscription (string | null)
    try {
      if (
        typeof req.body.token !== 'string' ||
        !(
      ...
      let db_res = await Database.updateAccountSubscription(
        result.account.id,
