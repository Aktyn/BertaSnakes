...
export async function handleMessage(
  ...
  switch (message.type) {
    ...
    case SOCIAL_CODES.REQUEST_FRIEND:
      {
        if (typeof message.friend_id !== 'string') break
        ...
        if (
          to_account_res.error !== ERROR_CODES.SUCCESS ||
          !to_account_res.data
        )
          break
        let to_account = to_account_res.data

        let res = await Database.insertAccountFriendRequest(
          from_id,
          to_account.id,
        )
        ...
        let potential_friend_connections = SocialConnection.getConnections(
          to_account.id,
        )
        if (potential_friend_connections) {
          ...
          if (get_public_data_res.data) {
            for (let potential_friend_conn of potential_friend_connections)
              potential_friend_conn.onFriendRequestReceived(
                get_public_data_res.data,
              )
          }
        }
      }
      break
    case SOCIAL_CODES.REMOVE_FRIEND:
      {
        //friend_id: string
        if (typeof message.friend_id !== 'string') break
        if (connection.removeFriend(message.friend_id)) {
          let friend_connections = SocialConnection.getConnections(
            message.friend_id,
          )
          ...
          let res = await Database.removeFriendship(
            message.friend_id,
      ...
      break
    case SOCIAL_CODES.ACCEPT_REQUEST:
      {
        //user_id: string
        if (typeof message.user_id !== 'string') break

        if (connection.getFriends().length >= Config.MAXIMUM_NUMBER_OF_FRIENDS)
          //some limits are necessary
          break

        let accept_db_res = await Database.acceptFriendship(
          connection.account.id,
          message.user_id,
        )
        if (accept_db_res.error !== ERROR_CODES.SUCCESS) break

        let friendship = await Database.getFriendshipID(
          connection.account.id,
          message.user_id,
        )
        if (!friendship) break

        let accepted_friend_connections = SocialConnection.getConnections(
          message.user_id,
        )
        connection.onRequestAccepted(
          message.user_id,
          friendship.id,
          friendship.left,
          accepted_friend_connections,
        )
      ...
      break
    case SOCIAL_CODES.REJECT_REQUEST:
      {
        //user_id: string
        if (typeof message.user_id !== 'string') break

        let reject_db_res = await Database.removeFriendship(
          message.user_id,
          connection.account.id,
        )
        if (reject_db_res.error !== ERROR_CODES.SUCCESS) break

        connection.onRequestRejected(message.user_id)

        let rejected_friend_connections = SocialConnection.getConnections(
          message.user_id,
        )
        if (rejected_friend_connections) {
          for (let rejected_friend_connection of rejected_friend_connections)
            rejected_friend_connection.onAccountRejectedFriendRequest(
              connection.account.id,
            )
        }
      }
      break
    case SOCIAL_CODES.REQUEST_CONVERSATION_DATA:
      {
        //friendship_id: string
        if (typeof message.friendship_id !== 'string') break
      ...
      break
    case SOCIAL_CODES.SEND_CHAT_MESSAGE:
      {
        ...
        if (!friend_data)
          //only messaging friend is allowed
          break

        let recipient_connections = SocialConnection.getConnections(
          message.recipient_id,
        )
        if (!recipient_connections) {
          //making sure this user exists and fetch subscription data at the same time
          //let db_res = await Database.getUserPublicData(message.recipient_id);
          //if(db_res.error !== ERROR_CODES.SUCCESS || !db_res.data)
          //	break;
          let db_res = await Database.getAccountSubscription(
            message.recipient_id,
          )
          if (db_res.error !== ERROR_CODES.SUCCESS) break
          if (db_res.subscription) {
            sendPushNotification(db_res.subscription, {
              title: `Message from ${trimString(
        ...
        if (recipient_connections) {
          ...
          let awaits = SocialConnection.awaiting_messages.get(
            message.recipient_id,
          )
          if (!awaits) {
            awaits = new Set()
