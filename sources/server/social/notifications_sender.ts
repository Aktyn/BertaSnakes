...
function registerTimestamp(author_id: string) {
  ...
  while (timestamps.length > TIMESTAMP_SAMPLES)
...
function canSendChatMessage(author_id: string) {
  ...
  return (
    timestamps.length < TIMESTAMP_SAMPLES ||
    Date.now() - timestamps[0] > 1000 * 60
  ) //one minute per 4 messages
}
...
export function sendPushNotification(
  ...
  try {
    if (!canSendChatMessage(data.author_id)) return
    ...
    webpush
      .sendNotification(pushSubscription, JSON.stringify(data))
      .catch(onError)
  } catch (e) {
    onError(e)
  }
