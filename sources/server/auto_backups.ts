...
function copyFolderSync(from: string, to: string) {
  //this function assumes that 'to' folder does not exists
  fs.mkdirSync(to)
  fs.readdirSync(from).forEach((element: string) => {
    if (fs.lstatSync(path.join(from, element)).isFile())
      fs.copyFileSync(path.join(from, element), path.join(to, element))
    else copyFolderSync(path.join(from, element), path.join(to, element))
  })
}
...
function removeDirectory(dir: string) {
  if (fs.existsSync(dir)) {
    fs.readdirSync(dir).forEach((entry: string) => {
      ...
      if (fs.lstatSync(entry_path).isDirectory()) removeDirectory(entry_path)
...
function getCurrentDatetimeCode() {
  //converts current date time to string of format: YYYYMMDDhhmmss
  let dt = new Date()
  return `${zeroPad(dt.getFullYear())}${zeroPad(dt.getMonth() + 1)}${zeroPad(
    dt.getDate(),
...
function clean() {
  //removes old backups if there are too many of them
  let backups = fs
    .readdirSync(BACKUPS_FOLDER)
    .sort((a: string, b: string) => a.localeCompare(b))

  while (backups.length > Config.MAXIMUM_BACKUPS) {
    let to_remove = backups.shift()
    if (to_remove) removeDirectory(path.join(BACKUPS_FOLDER, to_remove))
  }
}
...
function doBackup(repeat_after: number) {
  ...
  executeCommand(
    `mongodump -u ${credentials.USERNAME} -p ${credentials.PASSWORD} --authenticationDatabase=${credentials.USERNAME} -d ${credentials.DB_NAME} -o ${current_backup_path}`,
  ).catch(console.error)
  ...
  if (repeat_after > 0) {
    console.log(
      ...
      new Date(Date.now() + repeat_after).toLocaleString(),
    )
