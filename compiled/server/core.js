...
var Core = (function () {
    'use strict';
    var START_GAME_COUNTDOWN = 1; //seconds
    var MINIMUM_GAME_DURATION = 0 * 60;
    var MAXIMUM_GAME_DURATION = 30 * 60;
    var MAXIMUM_FRIENDS_NUMBER = 100;
    var MAX_LEVEL = 99;
    ...
    var NetworkCodes = require('./../include/network_codes.js');
    var RoomInfo = require('./../include/room_info.js');
    var UserInfo = require('./../include/user_info.js');
    var DatabaseUtils = require('./database_utils.js');
    var Maps = require('./../include/game/maps.js');
    var Player = require('./../include/game/objects/player.js');
    var Skills = require('./../include/game/common/skills.js');
    var GameStarter = require('./game_starter.js');
    ...
    function saveGameResult(room, result_json) {
        ...
        result_json.players_results.forEach(function (result) {
            if (result.user_id < 0) //ignore guests
                return;
            var online_user_conn = current_connections.find(function (conn) {
            ...
            if (online_user_conn !== undefined)
                updateAndSaveCustomData(online_user_conn.user.id, online_user_conn.user.custom_data, result);
            else { //update offline user according to game result
                DatabaseUtils.findUserByID(result.user_id).then(function (res) {
                    if (res === null)
    ...
    function startNewGame(room) {
        ...
        var confirmations = room.sits.slice(); //copy of array with id's of playing users (copy of sits)
        room.onUserConfirm = function (user_id) {
            if (user_id instanceof UserInfo)
                user_id = user_id.id;
            assert(typeof user_id === 'number', 'Incorrect argument type');
            console.log('\t' + user_id + ' confirmed');
            var index = confirmations.indexOf(user_id);
            if (index > -1)
        ...
        try {
            ...
            current_connections.forEach(function (conn) {
                ...
                switch (msg.action) {
                    case NetworkCodes.START_ROUND_ACTION:
                        { //@msg.data - game duration in seconds
                            var pass_data_1 = JSON.stringify(Object.assign({
                            ...
                            room.users.forEach(function (room_user) {
                                if (room_user.connection == null)
                                    throw new Error('room_user has not assigned connection handler');
                                room_user.connection.send(pass_data_1);
                            });
                        }
                        break;
                    ...
                    case NetworkCodes.END_GAME_ACTION:
                        {
                            var pass_data_2 = JSON.stringify(Object.assign({
                            ...
                            room.users.forEach(function (room_user) {
                                if (room_user.connection == null)
                                    throw new Error('room_user has not assigned connection handler');
                                room_user.connection.send(pass_data_2);
                            });
            ...
            room.game_process.send({
                ...
                users: room.users.filter(function (user) { return room.isUserSitting(user); })
                    .map(function (user) { return user.toFullJSON(); }),
                room_info: room.toJSON()
            });
            //console.log();
        }
    ...
    function getRoomByID(id) {
        for (var _i = 0, avaible_rooms_1 = avaible_rooms; _i < avaible_rooms_1.length; _i++) {
            var room = avaible_rooms_1[_i];
            if (room.id === id)
                return room;
        }
        return undefined;
    ...
    function removeRoom(room) {
        var index = avaible_rooms.indexOf(room);
        if (index > -1)
    ...
    function distributeAndUpdateAccountData(conn) {
        ...
        DatabaseUtils.updateUserCustomData(conn.user.id, JSON.stringify(conn.user.custom_data), conn.user.custom_data.rank | 0);
    }
    var handleJSON = function (connection, json_data) {
        switch (json_data.type) {
            ...
            case NetworkCodes.SUBSCRIBE_LOBBY_REQUEST:
                {
                    ...
                    if (connection.user !== null /* && connection.user.room === null*/) {
                        ...
                        connection.send(JSON.stringify({
                            ...
                            rooms: avaible_rooms.filter(function (room) { return room.game_process === null; })
                                .map(function (room) { return room.toJSON(); })
                        }));
                    }
                }
                break;
            case NetworkCodes.ACCOUNT_DATA_REQUEST:
                {
                    ...
                    var mapped = JSON.parse(JSON.stringify(connection.user.friends));
                    mapped.forEach(function (f) {
                        f.online = !!current_connections.find(function (conn) { return conn.user && conn.user.id === f.id; });
                    });
                    connection.send(JSON.stringify({
                        type: NetworkCodes.ACCOUNT_DATA,
            ...
            case NetworkCodes.SKILL_USE_REQUEST:
                {
                    ...
                    if (connection.user.isGuest() === false &&
                        ...
                        for (var i = 0; i < connection.user.custom_data.skills.length; i++) {
            ...
            case NetworkCodes.SKILL_PUT_OFF_REQUEST:
                ...
                break;
            case NetworkCodes.SKILLS_ORDER_REQUEST:
                { //@skills - array of skill indexes and nulls
                    ...
                    if (connection.user.isGuest() === true)
                        break;
                    var custom_data = connection.user.custom_data;
                    assert(json_data.skills.length === custom_data.skills.length, 'Different sizes of arrays of skills indexes');
                    for (var i = 0; i < json_data.skills.length; i++) {
                    ...
                    for (var i = 0; i < custom_data.skills.length; i++) {
                        for (var j = i + 1; j < custom_data.skills.length; j++) {
            ...
            case NetworkCodes.SHIP_BUY_REQUEST:
                {
                    ...
                    if (connection.user.isGuest() === true)
                        break;
                    var cost = Player.SHIP_COSTS[json_data.ship_type];
                    var lvl_requirement = Player.SHIP_LVL_REQUIREMENTS[json_data.ship_type];
            ...
            case NetworkCodes.SKILL_BUY_REQUEST:
                {
                    ...
                    if (connection.user.isGuest() === true)
                        break;
                    var skill = Object.values(Skills).find(function (s) {
            ...
            case NetworkCodes.USER_KICK_REQUEST:
                {
                    ...
                    if (connection.user.room.getOwner().id === connection.user.id &&
                        ...
                        var target_user = connection.user.room.getUserByID(json_data.user_id);
            ...
            case NetworkCodes.JOIN_ROOM_REQUEST:
                {
                    ...
                    var room = getRoomByID(json_data.id);
                    if (!room)
                    ...
                    if (connection.user.room !== null) {
                        if (connection.user.room.id === json_data.id) //same room - ignoring it
                            break;
                        var old_room_id = connection.user.room.id;
                        ...
                        connection.send(JSON.stringify({
                            ...
                            room_info: room.toJSON(),
                            users: room.users.map(function (user) { return user.toJSON(); })
                        }));
            ...
            case NetworkCodes.SIT_REQUEST:
                {
                    assert(connection.user != null, 'Connection\'s user doesn\'t exist');
                    assert(connection.user.room != null, 'User is not in room.');
                    var sit_req = json_data.type === NetworkCodes.SIT_REQUEST;
                    if (connection.user.room.isUserSitting(connection.user) !== sit_req) {
                        if (sit_req)
                            connection.user.room.sitUser(connection.user);
                        else
                            connection.user.room.standUpUser(connection.user);
            ...
            case NetworkCodes.READY_REQUEST:
                {
                    ...
                    if (connection.user.room.everyoneReady() === true) {
                        try {
                            GameStarter.start_countdown(connection.user.room, START_GAME_COUNTDOWN, startNewGame);
                        }
            ...
            case NetworkCodes.ROOM_UPDATE_REQUEST:
                {
                    ...
                    if (room.getOwner().id === connection.user.id) {
                        ...
                        assert(json_data.duration > MINIMUM_GAME_DURATION, 'duration must be longer or equal 3 minutes');
                        assert(json_data.duration <= MAXIMUM_GAME_DURATION, 'duration must be no longer than 30 minutes');
                        ...
                        var map = Maps.getByName(json_data.map);
                        if (map != null)
                            room.map = map.name;
                        //sending room update
                        distributeRoomUpdate(connection.user.room);
            ...
            case NetworkCodes.SEND_ROOM_MESSAGE:
                {
                    assert(connection.user != null, 'Connection\'s user doesn\'t exist');
                    assert(connection.user.room != null, 'User is not in room. Cannot send room chat message');
                    ...
                    var room_message_1 = JSON.stringify({
                        type: NetworkCodes.RECEIVE_CHAT_MESSAGE,
                        from: connection.user.nick,
                        public: true,
                        id: connection.user.room.id,
                        msg: json_data.msg
                    });
                    //send chat message to every user on room
                    connection.user.room.users.forEach(function (user) { return user.connection.send(room_message_1); });
                }
                break;
            ...
            case NetworkCodes.ADD_FRIEND_REQUEST:
                {
                    ...
                    //if already a friend
                    if (connection.user.friends.find(function (f) { return f.id === json_data.user_id; }) !== undefined)
                        break;
                    ...
                    for (var i = 0; i < current_connections.length; i++) {
                        if (current_connections[i].user !== null &&
                            ...
                            DatabaseUtils.updateFriendsList(connection.user.id, JSON.stringify(connection.user.friends));
            ...
            case NetworkCodes.REMOVE_FRIEND_REQUEST:
                {
                    assert(connection.user !== null, 'Connection\'s user doesn\'t exist');
                    assert(typeof json_data.user_id === 'number', 'user_id is not type of number');
                    for (var i = 0; i < connection.user.friends.length; i++) {
    ...
    return {
        addConnection: function (connection) {
            ...
            var session_key = crypto.createHash('sha256')
                .update(connection.req.headers['user-agent'] + connection.ip + key).digest('base64');
            // console.log(session_key);
            DatabaseUtils.checkSession(session_key).then(function (res) {
                if (res.length > 0) { //logged in as user
                    var new_id_1 = parseInt(res[0].id);
                    if (current_connections.some(function (conn) { return conn.user && conn.user.id === new_id_1; })) {
                        console.log('user already logged in - TODO - do something about it');
                        connection.close();
                        _this.removeConnection(connection);
                        return;
                    }
                    connection.user = new UserInfo(new_id_1, res[0].nickname, res[0].custom_data);
                    try {
                    ...
                    loginAsGuest(connection);
            }).catch(function (e) { return console.error('Session checking error', e); });
        },
        removeConnection: function (connection) {
            var index = current_connections.indexOf(connection);
            if (index > -1) {
                if (current_connections[index].user != null && current_connections[index].user.room)
                    removeUserFromRoom(current_connections[index].user, current_connections[index].user.room);
            ...
            try {
                if (typeof message === 'string') //stringified JSON object
                    ...
                    if (connection.user.room.game_process != null) {
                        connection.user.room.game_process.send({ user_id: connection.user.id, data: message });
                    }
                }
                else
