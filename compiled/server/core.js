...
const game_starter_1 = require("./game_starter");
...
function updateAndSaveCustomData(user_id, custom_data, result) {
    ...
    database_utils_1.default.updateUserCustomData(user_id, JSON.stringify(custom_data), custom_data.rank | 0);
}
function saveGameResult(room, result_json) {
    ...
    result_json.players_results.forEach(result => {
        ...
        if (online_user_conn !== undefined && online_user_conn.user !== null)
            ...
            database_utils_1.default.findUserByID(result.user_id).then(res => {
                if (res === null)
    ...
    database_utils_1.default.saveGameResult(room.name, room.map, room.gamemode, room.duration, result_json_out);
}
...
function startNewGame(room) {
    ...
    try {
        ...
        // test.on('message', )
        //game message
        room.game_process.on('message', (msg) => {
...
function distributeAndUpdateAccountData(conn) {
    if (conn.user === null)
        throw 'connection.user cannot be null';
    ...
    database_utils_1.default.updateUserCustomData(conn.user.id, JSON.stringify(conn.user.custom_data), conn.user.custom_data.rank | 0);
}
        ...
        case NetworkCodes.ACCOUNT_DATA_REQUEST:
            {
                ...
                mapped.forEach((f) => {
                    f.online = !!current_connections.find(conn => conn.user !== null && conn.user.id === f.id);
                });
                connection.send(JSON.stringify({
                    type: NetworkCodes.ACCOUNT_DATA,
        ...
        case NetworkCodes.SKILL_BUY_REQUEST:
            {
                ...
                const skill_name = Object.keys(Skills).find(s_key => typeof Skills[s_key] === 'object' &&
        ...
        case NetworkCodes.SIT_REQUEST:
            {
                ...
                if (connection.user.room.isUserSitting(connection.user.id) !== sit_req) {
                    if (sit_req)
                        connection.user.room.sitUser(connection.user.id);
                    else
                        connection.user.room.standUpUser(connection.user.id);
        ...
        case NetworkCodes.READY_REQUEST:
            {
                ...
                if (connection.user.room.everyoneReady() === true) {
                    try {
                        game_starter_1.default.start_countdown(connection.user.room, START_GAME_COUNTDOWN, startNewGame);
                    }
                ...
                for (let i = 0; i < current_connections.length; i++) {
                    ...
                    if (_conn !== null && _conn.id === json_data.user_id) {
                        ...
                        if (true) { //build dataset
                            ...
                            database_utils_1.default.customQuery("INSERT INTO `BertaBall`.`chat_dataset` \
        ...
        case NetworkCodes.ADD_FRIEND_REQUEST:
            {
                ...
                for (let i = 0; i < current_connections.length; i++) {
                    ...
                    if (_conn !== null && _conn.id === json_data.user_id) {
                        ...
                        database_utils_1.default.updateFriendsList(connection.user.id, JSON.stringify(connection.user.friends));
...
exports.default = {
    addConnection: function (connection) {
        ...
        database_utils_1.default.checkSession(session_key).then((res) => {
