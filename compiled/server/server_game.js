...
(function (ServerGame) {
    try {
        var _RoomInfo_ = require('./../include/room_info');
    ...
    var wave_i, chunk_it, chunk_ref, ss_i, obj_i, p_i, e_i, s_i, b_i, s_h, async_p_i, p_it, async_p_it, async_s, r_p_i, //e_h, e_h2
    hit_x, hit_y, offsets, sin, cos, synch_array;
    let emitAction = (action, data) => {
        try {
            //@ts-ignore
    ...
    const runLoop = (function () {
        ...
        //timing variables
        var hrtime, _dt = 0, _steps_, start, end, delta_sum = 0;
        var nano = function () {
    ...
    class Class extends GameCore {
        constructor(map, room) {
            super();
            this.respawning_players = [];
            this.dataForClients = [];
            this.time_to_enemy_wave = 0;
            this.time_to_item_spawn = 0;
            this.wave_number = 0;
            this.time_to_spawn = 0;
            this.spawn_timestamp = 0;
        ...
        onClientMessage(client_id, data) {
            ...
            switch (data[0]) {
                case NetworkCodes.PLAYER_MOVEMENT:
                    ...
                    emitAction(NetworkCodes.SEND_DATA_TO_CLIENT_ACTION_FLOAT32, [NetworkCodes.PLAYER_MOVEMENT_UPDATE, async_p_i,
                ...
                case NetworkCodes.PLAYER_SKILL_STOP_REQUEST:
                    ...
                    if (async_s) {
                        async_s.stopUsing();
                        emitAction(NetworkCodes.SEND_DATA_TO_CLIENT_ACTION_FLOAT32, [NetworkCodes.ON_PLAYER_SKILL_CANCEL, async_p_i, data[1]]);
                    }
                    break;
            }
        }
        //NOTE - this.bounceOutOfColor, not super !!!
        bouncePainter(object, color, bounce_vector) {
            //@ts-ignore
            return this.bounceOutOfColor(object, color, this, bounce_vector); //returns boolean
        }
        playerBounce(player, color) {
            if (this.bouncePainter(player, color, this.bounceVec) === true) {
                this.dataForClients.push(NetworkCodes.ON_PLAYER_BOUNCE, this.players.indexOf(player), player.x, player.y, player.rot, this.bounceVec.x, this.bounceVec.y);
                return true;
            }
            return false;
        }
        enemyBounce(enemy, color) {
            if (this.bouncePainter(enemy, color, this.bounceVec) === true) {
                this.dataForClients.push(NetworkCodes.ON_ENEMY_BOUNCE, enemy.id, enemy.x, enemy.y, enemy.rot, this.bounceVec.x, this.bounceVec.y);
                enemy.frames_since_last_update = 0;
            }
        }
        bulletBounce(bullet, color) {
            if (this.bouncePainter(bullet, color, this.bounceVec) === true) {
                this.dataForClients.push(NetworkCodes.ON_BULLET_BOUNCE, bullet.id, bullet.x, bullet.y, bullet.rot, this.bounceVec.x, this.bounceVec.y);
            }
        }
        onPlayerPainterCollision(player, color) {
            ...
            if (Colors.compareByteBuffers(Colors.WALLS.byte_buffer, color) === true) {
                if (player.spawning === true) { //pushing out of safe area and finishing spawning
                    this.bounceVec.set(player.x, player.y).normalize().scaleBy(GameCore.PARAMS.spawn_radius + GameCore.PARAMS.spawn_walls_thickness);
                    ...
                    this.dataForClients.push(NetworkCodes.ON_PLAYER_SPAWNING_FINISH, this.players.indexOf(player), player.x, player.y);
                }
                else
                ...
                if (player.effects.isActive(Effects.TYPES.POISONING) === false) {
                    ...
                    this.dataForClients.push(NetworkCodes.ON_PLAYER_POISONED, this.players.indexOf(player));
                }
            }
                ...
                if (this.room.gamemode === _RoomInfo_.MODES.COMPETITION) {
                    for (var player_col_i in Colors.PLAYERS_COLORS) {
                        if (Colors.compareByteBuffers(Colors.PLAYERS_COLORS[player_col_i].byte_buffer, color) === true) {
        ...
        onEnemyPainterCollision(enemy, color) {
            if (Colors.compareByteBuffers(Colors.WALLS.byte_buffer, color) === true) {
                this.enemyBounce(enemy, color);
            }
        }
        onBulletPainterCollision(bullet, color) {
        ...
        onPlayerEnemyCollision(player, enemy) {
            ...
            enemy.expired = true;
            this.dataForClients.push(NetworkCodes.ON_PLAYER_ENEMY_COLLISION, enemy.id, this.players.indexOf(player), player.x, player.y, player.rot, player.hp, player.points, this.bounceVec.x, this.bounceVec.y);
            if (player.isAlive() === false)
                this.onPlayerDeath(player, 0);
        }
        onPlayerEnemySpawnerCollision(player, spawner) {
            //@ts-ignore
            this.bounceOneObjectFromAnother(player, spawner);
            this.bounceVec.set(player.x - spawner.x, player.y - spawner.y).normalize();
            this.dataForClients.push(NetworkCodes.ON_PLAYER_BOUNCE, this.players.indexOf(player), player.x, player.y, player.rot, this.bounceVec.x, this.bounceVec.y);
        }
        onEnemyEnemySpawnerCollision(enemy, spawner) {
            //@ts-ignore
        ...
        onPlayerBulletCollision(player, bullet) {
            ...
            this.onPlayerAttackedPlayer(bullet.parent, player, damage);
            this.dataForClients.push(NetworkCodes.ON_BULLET_HIT, bullet.id, hit_x, hit_y);
            bullet.expired = true;
        }
        onEnemyBulletCollision(enemy, bullet) {
            ...
            this.onPlayerAttackedEnemy(bullet.parent, enemy, damage);
            //bullet dies on hit
            this.dataForClients.push(NetworkCodes.ON_BULLET_HIT, bullet.id, hit_x, hit_y);
            bullet.expired = true;
        }
        onPlayerItemCollision(player, item) {
            ...
            this.dataForClients.push(NetworkCodes.ON_PLAYER_COLLECT_ITEM, item.id, item.type, this.players.indexOf(player));
            item.expired = true;
        }
        onPlayerEnemyPainterCollision(player) {
            if (player.effects.isActive(Effects.TYPES.SHIELD) === false &&
                player.effects.isActive(Effects.TYPES.SPAWN_IMMUNITY) === false) {
            ...
            player.movement.speed = player.movement.maxSpeed;
            this.dataForClients.push(NetworkCodes.ON_PLAYER_ENEMY_PAINTER_COLLISION, this.players.indexOf(player), player.x, player.y, player.hp, player.points);
            super.paintHole(player.x, player.y, GameCore.PARAMS.small_explosion_radius);
        }
        onPlayerDeath(player, explosion_radius) {
            this.dataForClients.push(NetworkCodes.ON_PLAYER_DEATH, this.players.indexOf(player), RESPAWN_DURATION, player.x, player.y, explosion_radius);
        ...
        onPlayerAttackedPlayer(attacker, victim, damage) {
            if (this.room.gamemode !== _RoomInfo_.MODES.COMPETITION)
                return;
            victim.hp -= damage; //must be before putting data for clients
            this.dataForClients.push(NetworkCodes.ON_PLAYER_ATTACKED, this.players.indexOf(attacker), damage, this.players.indexOf(victim), victim.hp, victim.x, victim.y);
        ...
        onPlayerAttackedEnemy(player, enemy, damage) {
            enemy.hp_bar.hp -= damage; //must be before putting data for clients
            this.dataForClients.push(NetworkCodes.ON_ENEMY_ATTACKED, enemy.id, damage, this.players.indexOf(player), enemy.hp_bar.hp, enemy.x, enemy.y);
        ...
        onBombExplosion(bomb) {
            ...
            for (e_i = 0; e_i < this.enemies.length; e_i++) {
                if (this.enemies[e_i].spawning === false &&
                    ...
                    //NOTE - 1.0 == 100% damage
                    this.onPlayerAttackedEnemy(bomb.parent, this.enemies[e_i], 1.0);
                }
            }
            if (this.room.gamemode === _RoomInfo_.MODES.COMPETITION) {
        ...
        onStain(enemy) {
            ...
            this.dataForClients.push(NetworkCodes.ON_POISON_STAIN, stain_index, enemy.x, enemy.y, enemy.width * GameCore.PARAMS.stain_shrink);
        }
        initWave() {
            ...
            if (this.enemies.length < this.maximum_enemies) {
                ...
                for (wave_i = 0; wave_i < ENEMIES_PER_WAVE * this.players.length; wave_i++) {
                    ...
                    if (enemy === null)
                        continue;
                    //console.log(enemy);
                    this.dataForClients.push(NetworkCodes.SPAWN_ENEMY, enemy_class_index, enemy.id, enemy.x, enemy.y, enemy.rot);
        ...
        applySkillEffect(player, skill, player_i, skill_i, immediately_response) {
            ...
            switch (skill.data) {
                ...
                case Skills.SHOOT3:
                    ...
                    this.dataForClients.push(NetworkCodes.ON_BULLET_SHOT, player_i, offsets.length);
                    for (let i in offsets) {
                        sin = Math.sin(-player.rot);
                        cos = Math.cos(-player.rot);
                        let bullet = new Bullet((offsets[i].x * cos - offsets[i].y * sin) * player.width + player.x, (offsets[i].x * sin + offsets[i].y * cos) * player.height + player.y, player.rot, player //.painter.color
                        );
                ...
                case Skills.BOUNCE_SHOT:
                    ...
                    sin = Math.sin(-player.rot);
                    cos = Math.cos(-player.rot);
                    let bullet = new Bullet(-sin * player.width + player.x, cos * player.height + player.y, player.rot, player, //.painter.color,
                    true // bouncing
                    );
                    this.bullets.push(bullet);
                    //player_index, bullet_id, pos_x, pos_y, rot
                    this.dataForClients.push(NetworkCodes.ON_BOUNCE_BULLET_SHOT, player_i, bullet.id, bullet.x, bullet.y, bullet.rot);
                    break;
                case Skills.ENERGY_BLAST:
                    // super.paintHole(player.x, player.y, GameCore.PARAMS.energy_blast_radius);
                    this.dataForClients.push(NetworkCodes.ON_ENERGY_BLAST, player.x, player.y, Colors.PLAYERS_COLORS.indexOf(player.painter.color));
                    const radius_pow = pow(GameCore.PARAMS.energy_blast_radius);
                    for (e_i = 0; e_i < this.enemies.length; e_i++) {
                        //@ts-ignore
                        if (this.enemies[e_i].spawning === false &&
                            Vector.distanceSqrt(this.enemies[e_i], player) <= radius_pow) {
                            this.onPlayerAttackedEnemy(player, this.enemies[e_i], GameCore.PARAMS.energy_blast_damage);
                        }
                    }
                    if (this.room.gamemode === _RoomInfo_.MODES.COMPETITION) {
                        for (p_i = 0; p_i < this.players.length; p_i++) {
                            ...
                            if (this.players[p_i] !== player && this.players[p_i].spawning === false &&
                                ...
                                this.onPlayerAttackedPlayer(player, this.players[p_i], GameCore.PARAMS.energy_blast_damage);
                            }
                        }
                    }
                    break;
                ...
                case Skills.BOMB:
                    ...
                    this.dataForClients.push(NetworkCodes.ON_BOMB_PLACED, player_i, bomb.id, player.x, player.y);
                    break;
            }
            ...
            if (immediately_response) {
                emitAction(NetworkCodes.SEND_DATA_TO_CLIENT_ACTION_FLOAT32, [NetworkCodes.ON_PLAYER_SKILL_USE, player_i,
                    skill_i, player.energy]);
            }
            else
                this.dataForClients.push(NetworkCodes.ON_PLAYER_SKILL_USE, player_i, skill_i, player.energy);
        }
        updateSpawnings(delta) {
            //first time spawning
            if (this.time_to_spawn !== 0) {
                ...
                if (this.time_to_spawn <= 0) {
                    ...
                    for (p_i = 0; p_i < this.players.length; p_i++) {
                        p_it = this.players[p_i];
                        p_it.movement.setMaxSpeed();
                        this.dataForClients.push(NetworkCodes.PLAYER_MOVEMENT_UPDATE, p_i, p_it.rot, p_it.movement.state, p_it.movement.speed);
                    }
                    this.time_to_spawn = 0;
                }
            }
            ...
            for (r_p_i = 0; r_p_i < this.respawning_players.length; r_p_i++) {
                if ((this.respawning_players[r_p_i].time -= delta) <= 0) { //spawning finished
                    ...
                    p_it.movement.setMaxSpeed();
                    this.dataForClients.push(NetworkCodes.PLAYER_MOVEMENT_UPDATE, this.players.indexOf(p_it), p_it.rot, p_it.movement.state, p_it.movement.speed);
                    this.dataForClients.push(NetworkCodes.ON_IMMUNITY_EFFECT, this.players.indexOf(p_it));
        ...
        updateChunks() {
            for (chunk_it = 0; chunk_it < this.chunks.length; chunk_it++) {
                ...
                if (chunk_ref.need_update) {
                    ...
                    chunk_ref.buff = chunk_ref.ctx.getImageData(0, 0, chunk_ref.canvas.width, chunk_ref.canvas.height);
                }
            }
        }
        ...
        update(delta) {
            ...
            for (b_i = 0; b_i < this.bombs.length; b_i++) {
                if (this.bombs[b_i].expired === true) //bobm just exploded
                    this.onBombExplosion(this.bombs[b_i]);
            }
            super.update(delta);
            //@ts-ignore
            ...
            for (ss_i = 0; ss_i < this.server_synchronized.length; ss_i++) {
                ...
                for (obj_i = 0; obj_i < synch_array.length; obj_i++) {
                    if ((++synch_array[obj_i].frames_since_last_update) > SYNC_EVERY_N_FRAMES) {
                        ...
                        this.dataForClients.push(NetworkCodes.OBJECT_SYNCHRONIZE, synch_array[obj_i].id, ss_i, synch_array[obj_i].x, synch_array[obj_i].y, synch_array[obj_i].rot);
                    }
                }
            }
            for (p_i = 0; p_i < this.players.length; p_i++) { //for each player
                ...
                var segmentLength = Math.hypot(p_it.x - p_it.painter.lastPos.x, p_it.y - p_it.painter.lastPos.y);
                //if player moved minimum distance
                if (p_it.painter.active && segmentLength > p_it.width / 2.0) {
                    super.color = p_it.painter.color.hex;
                    super.drawLine(p_it.x, p_it.y, p_it.painter.lastPos.x, p_it.painter.lastPos.y, p_it.painter.thickness);
                    //add player index, player position x and y, player painter last position x and y,
                    //player painter thickness
                    this.dataForClients.push(NetworkCodes.DRAW_PLAYER_LINE, p_i, p_it.x, p_it.y, p_it.painter.lastPos.x, p_it.painter.lastPos.y);
    ...
    ServerGame.Class = Class;
})(ServerGame || (ServerGame = {})); //)();
exports.default = ServerGame;
