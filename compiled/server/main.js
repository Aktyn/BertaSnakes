...
const websock = new ws_1.Server({ port: PORT });
websock.on('connection', function (ws, req) {
    ...
    let connection = new connection_1.default(ws, req);
    core_1.default.addConnection(connection);
    ws.on('message', function (message) {
        core_1.default.onMessage(connection, message);
    });
    ws.on('close', () => {
        console.log('connection close:', connection.id);
        core_1.default.removeConnection(connection);
    });
});
