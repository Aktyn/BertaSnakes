...
export default class UserInfo {
  ...
  constructor(
    _id: number,
    _custom_data: UserPublicData | UserCustomData,
    _account_id?: string,
    ...
    if (this.id === 0)
      throw new Error(
        'Creating guest this way is deprecated. Use UserInfo.createGuest() instead',
      )
  ...
  public static fromJSON(
    json_data:
      | string
      | { id: number; account_id?: string; data: UserPublicData },
  ) {
    if (typeof json_data === 'string')
      json_data = <{ id: number; account_id?: string; data: UserPublicData }>(
  ...
  static fromFullJSON(full_json_data: string | UserFullData) {
    if (typeof full_json_data === 'string') {
      full_json_data = <
        { id: number; account_id?: string; data: UserCustomData }
      >JSON.parse(full_json_data)
    }
