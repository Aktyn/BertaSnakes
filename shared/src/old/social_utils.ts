...
function getPreviousMsgIndex(arr: SocialMessage[], timestamp: number) {
  for (let i = arr.length - 1; i >= 0; i--) {
    if (arr[i].timestamp < timestamp) return i
  }
...
export function pushSocialMessage(arr: SocialMessage[], msg: SocialMessage) {
  ...
  if (
    last_i !== -1 &&
    arr[last_i].left === msg.left &&
    msg.timestamp - arr[last_i].timestamp < 1000 * 60
  )
    ...
    if (!_CLIENT_) {
      //shift array if it is getting too big (server-side only)
      while (arr.length > Config.MAXIMUM_LENGTH_OF_MESSAGES_CHUNK) arr.shift()
    }
  }
}
