...
import type { UserPublicData } from './user_info'
...
export default class RoomInfo {
  ...
  public getUsersPublicData() {
    const data: { id: number; data: UserPublicData }[] = []
  ...
  static fromJSON(json_data: string | RoomCustomData) {
    ...
    const room = new RoomInfo(json_data['id'], json_data['name'])
  ...
  removeUser(user: UserInfo) {
    ...
    if (this.sits.some((s) => s === user.id)) this.standUpUser(user)
  ...
  getTakenSits() {
    let counter = 0
    for (const sit of this.sits) if (sit !== 0) counter++
    return counter
    //return this.sits.filter(sit => sit !== 0).length || 0;
  }

  getOwner(): UserInfo | undefined {
    //returns room owner (first user in list)
  ...
  isUserReady(user_id: number) {
    const sit_id = this.sits.indexOf(user_id)
  ...
  updateSettings(settings: RoomSettings) {
    this.duration = Math.max(
      Config.MINIMUM_GAME_DURATION,
      Math.min(Config.MAXIMUM_GAME_DURATION, settings.duration),
    )
    this.max_enemies = Math.max(
      10,
      Math.min(Config.MAXIMUM_ENEMIES_LIMIT, settings.max_enemies),
    )
    this.gamemode = Math.max(
      0,
      Math.min(GAMEMODES_COUNT - 1, settings.gamemode),
    )
    this.name = settings.name.substr(0, Config.MAXIMUM_ROOM_NAME_LENGTH)

    const min_sits = this.gamemode === GAME_MODES.COMPETITION ? 2 : 1
    const new_sits_count = Math.max(
      min_sits,
      Math.min(Config.MAXIMUM_SITS, settings.sits_number),
    )
    this.changeSitsNumber(new_sits_count)

    if (settings.map in Maps) this.map = settings.map
  }
  ...
  private changeSitsNumber(sits_number: number) {
    while (this.sits.length > sits_number)
      //removing last sits
      this.sits.pop()
    while (this.sits.length < sits_number) this.sits.push(0)
  ...
  updateData(json_data: RoomInfo | string | RoomCustomData) {
    if (json_data instanceof RoomInfo) {
      //update from RoomInfo instance
      if (this.id !== json_data.id) throw Error('id mismatch')
  ...
  standUpUser(user: UserInfo) {
    this.sits = this.sits
      .map((sit) => (sit === user.id ? 0 : sit))
      .sort((a) => (a === 0 ? 1 : -1))
    this.unreadyAll()
  }

  unreadyAll() {
    for (let i = 0; i < this.readys.length; i++) this.readys[i] = false
  }

  setUserReady(user: UserInfo) {
    if (!this.sits.every((sit) => sit !== 0))
      //not every sit taken
      return false
    for (let i = 0; i < this.sits.length; i++) {
