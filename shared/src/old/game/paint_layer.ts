...
let sxi_temp: number,
  ...
  itX: number,
  ch_i: number,
  chunk_ctx: CanvasRenderingContext2D,
  relXs: number,
  relYs: number,
  relXe: number,
  relYe: number,
...
export default class PaintLayer {
  ...
  destroy() {
    this.chunks.forEach((ch) => {
      if (ch.webgl_texture !== null) ch.webgl_texture.destroy()
      delete ch.canvas
      delete ch.buff
    })
    //@ts-ignore
  ...
  generateChunks(res: PAINTER_RESOLUTION) {
    ...
    if (!this.size) throw new Error('No size specified for number chunks')
    ...
    let chunks_memory =
      2 *
      ((this.size * this.size * this.CHUNK_RES * this.CHUNK_RES * 4) /
        (1024 * 1024))
    console.log(
      'map size:',
      this.map_size,
    ...
    for (let y = 0; y < this.size; y++) {
      for (let x = 0; x < this.size; x++) {
        ...
        xx =
          -(this.size - 1) * PaintLayer.CHUNK_SIZE +
          x * PaintLayer.CHUNK_SIZE * 2
        yy =
          (this.size - 1) * PaintLayer.CHUNK_SIZE -
          y * PaintLayer.CHUNK_SIZE * 2
        ...
        let ctx = <CanvasRenderingContext2D>(
          canvas.getContext('2d', { antialias: true, alpha: true })
        )
  ...
  drawLine(
    ...
    for (itY = syi; itY <= eyi; itY++) {
      for (itX = sxi; itX <= exi; itX++) {
        ...
        relXs =
          ((sx +
            this.size * PaintLayer.CHUNK_SIZE -
            itX * PaintLayer.CHUNK_SIZE * 2) /
            (PaintLayer.CHUNK_SIZE * 2.0)) *
          this.CHUNK_RES
        relYs =
          ((-sy +
            this.size * PaintLayer.CHUNK_SIZE -
            itY * PaintLayer.CHUNK_SIZE * 2) /
            (PaintLayer.CHUNK_SIZE * 2.0)) *
          this.CHUNK_RES
        ...
        if (!prevent_chunks_update) this.chunks[ch_i].need_update = true
  ...
  paintMapWalls(map: MapJSON_I) {
    ...
    if (_CLIENT_) {
      ...
      map_canvas = Canvas.createCanvas(
        this.CHUNK_RES * this.size,
        this.CHUNK_RES * this.size,
      )

    let map_ctx = <CanvasRenderingContext2D>(
      map_canvas.getContext('2d', { antialias: true, alpha: true })
    )
    if (!_CLIENT_) {
      //@ts-ignore
    ...
    let canvasData = map_ctx.getImageData(
        0,
        0,
        map_canvas.width,
        map_canvas.height,
    ...
    if (typeof map_canvas.remove === 'function') map_canvas.remove()
  }
  ...
  drawWalls(
    ...
    if (restrict === undefined) restrict = 0x0f //binary -> 00001111

    //top
    if (restrict & (1 << 0))
      this.drawLine(
        -this.map_size,
        this.map_size - thickness,
        this.map_size,
        this.map_size - thickness,
        thickness,
        prevent_chunks_update,
      )
    //bottom
    if (restrict & (1 << 1))
      this.drawLine(
  ...
  paintHole(sx: number, sy: number, radius: number) {
    ...
    if (bytes !== 0) this.drawWalls(this.walls_thickness, bytes, true)

    //checking spawn
    if (
      pow2(sx) + pow2(sy) <=
      pow2(this.spawn_radius + this.spawn_thickness + radius)
    ) {
  ...
  getPixelColor(x: number, y: number, out_buff: Uint8Array) {
    ...
    if (this.chunks[ch_i] /* && this.chunks[ch_i].buff != null*/) {
      relXs =
        ((x +
          this.size * PaintLayer.CHUNK_SIZE -
          sxi * PaintLayer.CHUNK_SIZE * 2) /
          (PaintLayer.CHUNK_SIZE * 2.0)) *
        this.CHUNK_RES
      relYs =
        ((-y +
          this.size * PaintLayer.CHUNK_SIZE -
          syi * PaintLayer.CHUNK_SIZE * 2) /
          (PaintLayer.CHUNK_SIZE * 2.0)) *
        this.CHUNK_RES
