...
export class SkillObject {
  ...
  canBeUsed(available_energy: number) {
    return (
      available_energy + 0.001 >= this.data.energy_cost && this.cooldown <= 0
    )
    // && this._in_use === false;
  }
  ...
  use() {
    //returns value of consumed energy
...
const Skills: SkillsSchema = {
  ...
  SHIELD: <SkillData>{
    ...
    name: 'Shield',
    description:
      'Active shield that protects player from damage for some time.',
    lvl_required: 3,
    price: 500, //coins
  },
  ...
  BOMB: <SkillData>{
    ...
    name: 'Bomb',
    description:
      'Place a bomb that explodes a while later killing every nearby enemy.',
    lvl_required: 9,
    price: 8000,
  },
...
if (_CLIENT_) console.log('Total available skills:', i)
