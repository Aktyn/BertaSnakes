const toHexString = (number: number) =>
  '#' + ('000000' + number.toString(16)).substr(-6)
...
interface ColorsSchema extends PaletteSchema {
  compareByteBuffers(
    buff1: Uint8Array | number[],
    buff2: Uint8Array | number[],
  ): boolean
...
const Colors = {
  ...
  isPlayerColor: function (buff) {
    for (
      let player_col_i = 0;
      player_col_i < (<ColorI[]>PALETTE.PLAYERS_COLORS).length;
      player_col_i++
    ) {
      if (
        Colors.compareByteBuffers(
          (<ColorI[]>PALETTE.PLAYERS_COLORS)[player_col_i].byte_buffer,
          buff,
        )
      )
        return true
