...
function loadImage(path: string | null) {
  ...
  img.onload = () => {
    pending--
    if (isReady()) on_load_listeners.forEach((l) => l())
  }
...
export function onMapsLoaded(callback: () => void) {
  if (isReady()) callback()
  else on_load_listeners.push(callback)
}
...
const Maps = {
  Empty: {
    ...
    background_texture: loadImage(
      _CLIENT_ ? require('../maps/bg_flat.jpg') : null,
    ),
    walls_texture: loadImage(
      _CLIENT_
        ? require('../maps/Empty.png')
        : path.join(MAP_FOLDER, 'Empty.png'),
    ),
  } as MapJSON_I,
  'Open Maze': {
    map_size: 5,
    ...
    walls_texture: loadImage(
      _CLIENT_
        ? require('../maps/OpenMaze.svg')
        : path.join(MAP_FOLDER, 'OpenMaze.png'),
    ),
  } as MapJSON_I,
  'Simple Maze': {
    map_size: 5,
    ...
    walls_texture: loadImage(
      _CLIENT_
        ? require('../maps/SimpleMaze.png')
        : path.join(MAP_FOLDER, 'SimpleMaze.png'),
    ),
  } as MapJSON_I,
  Snowflake: {
    map_size: 5,
    ...
    walls_texture: loadImage(
      _CLIENT_
        ? require('../maps/Snowflake.svg')
        : path.join(MAP_FOLDER, 'Snowflake.png'),
    ),
  } as MapJSON_I,
  Checkers: {
    map_size: 5,
    ...
    walls_texture: loadImage(
      _CLIENT_
        ? require('../maps/Checkers.svg')
        : path.join(MAP_FOLDER, 'Checkers.png'),
    ),
  } as MapJSON_I,
}
