...
import { GAME_MODES } from '../room-info'
...
const abstractFunc_painter: PainterCollisionListener = function (
  object: Object2D,
  pixel_buffer: Uint8Array,
) {}
// noinspection JSUnusedLocalSymbols
const abstractFunc_object_to_object: ObjectCollisionListener = function (
  arg1: Object2D,
  arg2: Object2D,
) {}
...
let cm_i
const colorsMatch = (c1: Uint8Array, c2: Uint8Array) => {
  //@c1, c2 - Uint8Array buffers of size 4
  for (cm_i = 0; cm_i < 4; cm_i++) {
    if (c1[cm_i] != c2[cm_i]) return false
  }
  return true
}
...
let find_trials,
  ...
  temp_arr,
  overlap_ray_steps,
  overlap_ray_color = new Uint8Array(4),
  o_rr,
  o_angle,
  o_a_s,
  o_dx,
...
function bounceOutOfColor(
  ...
  bounce_vec.set(0, 0)
  if (!getBounceVec(object, color, map, bounce_vec))
    //no collision detected
    return false

  bounce_vec.normalize()

  if (out_bounce_vec != null) out_bounce_vec.set(bounce_vec.x, bounce_vec.y)
  ...
  current_vec.set(
    Math.cos(-object.rot + Math.PI / 2.0),
    Math.sin(-object.rot + Math.PI / 2.0),
  )
  ...
  if (b_product > 0.0) return true /*NOTE - changed from false*/
...
function bounceOneObjectFromAnother(obj1: Object2D, obj2: Object2D) {
  current_vec
    .set(
      Math.cos(-obj1.rot + Math.PI / 2.0),
      Math.sin(-obj1.rot + Math.PI / 2.0),
    )
    .normalize()
...
function paintOverlap(map: any, in_vec: Vector, _radius: number) {
  ...
  for (o_r_s = overlap_ray_steps; o_r_s > 0; o_r_s--) {
    ...
    for (o_a_s = 0; o_a_s < OVERLAP_ANGLE_STEPS; o_a_s++) {
      ...
      if (overlap_ray_color[3] > 0)
        //pixel is not invisible => overlap
        return true
...
export default {
  ...
  detectCollisions: function (map: GameMap, gamemode: GAME_MODES) {
    for (p_i = 0; p_i < map.players.length; p_i++) {
      //for each player
      //player to painter collision
      this.detectSensorToPainterCollision(
        map,
        map.players[p_i],
        this.onPlayerPainterCollision,
      )
      ...
      for (es_i = 0; es_i < map.enemy_spawners.length; es_i++) {
        if (twoObjectsIntersect(map.players[p_i], map.enemy_spawners[es_i]))
          this.onPlayerEnemySpawnerCollision(
            map.players[p_i],
    ...
    for (e_i = 0; e_i < map.enemies.length; e_i++) {
      ...
      this.detectSensorToPainterCollision(
        map,
        map.enemies[e_i],
        this.onEnemyPainterCollision,
      )

      if (!map.enemies[e_i].spawning) {
        //only spawned enemies

        //enemy to enemy spawner collision
        for (es_i = 0; es_i < map.enemy_spawners.length; es_i++) {
          if (twoObjectsIntersect(map.enemies[e_i], map.enemy_spawners[es_i]))
            this.onEnemyEnemySpawnerCollision(
              map.enemies[e_i],
              map.enemy_spawners[es_i],
            )
        }
    ...
    for (b_i = 0; b_i < map.bullets.length; b_i++) {
      ...
      this.detectSensorToPainterCollision(
        map,
        map.bullets[b_i],
        this.onBulletPainterCollision,
      )
    }
  },
  detectSensorToPainterCollision: function (
    map: GameMap,
    object: Object2D,
    onCollide: PainterCollisionListener,
  ) {
    //@ts-ignore
    if (object.sensor === undefined) console.log(object)
    //@ts-ignore
  ...
  findRandomEmptySpot: function (
    ...
    while (find_trials++ < 16) {
      //maximum trials for performance matter
      out_vec.set(randRange(-sc, sc), randRange(-sc, sc))

      if (
        /*distanceSqrt(out_vec.x(), out_vec.y(), 0, 0) //TODO - check distance to safe area
				> pow2f(SAFE_AREA_RADIUS+_radius)
				&&*/ out_vec.x > -sc + wall_margin &&
        out_vec.x < sc - wall_margin &&
        out_vec.y > -sc + wall_margin &&
        out_vec.y < sc - wall_margin &&
