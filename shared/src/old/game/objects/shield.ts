  ...
  destroy() {
    ...
    if (typeof EntitiesBase !== 'undefined')
      ...
      EntitiesBase.removeObject(
        EntitiesBase.getEntityId(this.entity_name as string),
        this,
      )
  }

  update(delta: number) {
    if ((this.timer += delta) >= this.duration) this.expired = true

    if (this.timer <= GROWING_TIME)
      sc = (this.timer / GROWING_TIME) * this.target_scale
    else if (this.duration - this.timer < SHRINKING_TIME)
      sc =
        Math.pow((this.duration - this.timer) / SHRINKING_TIME, 0.125) *
        this.target_scale
    else sc = this.target_scale
