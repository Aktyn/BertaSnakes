...
export default class Object2DSmooth extends Object2D {
  ...
  setPos(x: number, y: number, do_not_smooth = false) {
    this.actual_x = x
    this.actual_y = y
    if (do_not_smooth) super.setPos(x, y)
    return this
  }
  ...
  move(x: number, y: number, do_not_smooth = false) {
    this.actual_x += x
    this.actual_y += y
    if (do_not_smooth) super.move(x, y)
    return this
  }
  ...
  setRot(rot: number, do_not_smooth = false) {
    this.actual_rot = rot
    if (do_not_smooth) super.setRot(rot)
    return this
  }
  set rot(rot) {
    this.actual_rot = rot
  ...
  update(delta: number) {
    ...
    if (rot_dt > Math.PI) rot_dt -= Math.PI * 2.0
    else if (rot_dt < -Math.PI) rot_dt += Math.PI * 2.0

    super.setRot(
      this._rot +
        rot_dt * delta * Object2DSmooth.SMOOTHNESS * Math.abs(rot_dt * 2.0),
    )
    while (this._rot < 0) super.setRot(this._rot + Object2DSmooth.PI_2)
