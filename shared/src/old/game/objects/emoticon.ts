...
const SCALE = 0.07,
  DURATION = 2,
  FADING_DURATION = 0.5 //durations in seconds
const OFFSET_ANGLE = Math.PI / 4.0,
  PARENT_OFFSET = 0.15

let sc = 0

export const EMOTS = [
  //NOTE - use uppercase letters for key values
...
export default class Emoticon extends Object2D {
  ...
  constructor(name: string, parent: Object2D, _entitiesClass?: any) {
    ...
    if (this.entitiesClass) {
      this.entitiesClass.addObject(
        this.entitiesClass.getEntityId('STREAK'),
        this.streak,
      )
      this.entitiesClass.addObject(
        this.entitiesClass.getEntityId(Emoticon.entityName(this.name)),
        this,
      )
    }
  }
  ...
  update(delta: number) {
    if ((this.timer += delta) > DURATION) this.expired = true

    if (this.factor < 1) {
      //showing up factor
      if ((this.factor += delta * 3.0) > 1) this.factor = 1
    }

    if (DURATION - this.timer < FADING_DURATION)
      sc = Math.pow((DURATION - this.timer) / FADING_DURATION, 2) * SCALE
    else sc = Math.pow(this.factor, 2) * SCALE

    super.setPos(
      this.parent.x +
        Math.cos(OFFSET_ANGLE) * Math.pow(this.factor, 2) * PARENT_OFFSET,
      this.parent.y +
        Math.sin(OFFSET_ANGLE) * Math.pow(this.factor, 2) * PARENT_OFFSET,
    )
    this.streak.setPos(
