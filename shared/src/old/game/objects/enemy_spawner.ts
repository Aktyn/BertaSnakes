...
if (_CLIENT_) {
  ...
  var EntitiesBase = require('../../../client/game/entities').default,
    ...
    SpawnerEmitter =
      require('../../../client/game/emitters/spawner_emitter').default
}

const SCALE = 0.15,
  GROWING_TIME = 1,
  SHRINKING_TIME = 1,
  ENEMY_GROWING_TIME = 0.5,
  GAP_TIME = 2.0
const ENTITY_NAME = 'ENEMY_SPAWNER',
  POISONOUS_ENTITY_NAME = 'POISONOUS_ENEMY_SPAWNER'
...
export default class EnemySpawner extends Object2D {
  ...
  constructor(enemy: Enemy) {
    ...
    if (typeof EntitiesBase !== 'undefined') {
      this.entity_name =
        enemy instanceof PoisonousEnemy ? POISONOUS_ENTITY_NAME : ENTITY_NAME
      //@ts-ignore
    ...
    //@ts-ignore
    if (
      typeof RendererBase !== 'undefined' &&
      typeof SpawnerEmitter !== 'undefined' &&
      //@ts-ignore
  ...
  update(delta: number) {
    ...
    switch (this.state) {
      case 0:
        ...
        break
      case 1:
        if (this.timer >= GAP_TIME) this.nextState(GAP_TIME)
        break
      case 2:
        {
          //popping up enemy
