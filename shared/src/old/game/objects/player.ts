...
if (_CLIENT_) {
  ...
  var PlayerEmitter =
    require('../../../client/game/emitters/player_emitter').default
  // noinspection ES6ConvertVarToLetConst
  var PoisoningEmitter =
    require('../../../client/game/emitters/poisoning_emitter').default
}
...
const PLAYER_SENSOR_SHAPES: number[][][] = [
  SENSOR_SHAPES.TRIANGLE,
  SENSOR_SHAPES.SQUARE,
  SENSOR_SHAPES.PENTAGON,
]
...
const bullets_offsets_2 = [
  { x: -0.5, y: 1 },
  { x: 0.5, y: 1 },
]
const bullets_offsets_3 = [
  { x: 0, y: 1 },
  { x: -0.5, y: 0.5 },
  { x: 0.5, y: 0.5 },
]
...
export default class Player extends _ExtendClass_ {
  ...
  constructor(
    ...
    if (this.entitiesClass) {
      ...
      this.entitiesClass.addObject(
        this.entitiesClass.getEntityId(this.entity_name),
        this,
      )
    }
  ...
  destroy() {
    ...
    if (this.emitter) this.emitter.expired = true
    if (this.poisoning_emitter) this.poisoning_emitter.expired = true
  }

  onPoisoned() {
    //client-side only use for poisoning particle effect display
    if (this.poisoning_emitter === null) {
      if (this.rendererClass)
        this.poisoning_emitter = this.rendererClass.addEmitter(
          new PoisoningEmitter(this),
        )
    } else this.poisoning_emitter.resetTimer()
  }
  ...
  shootBullets(
    skill_data:
      | typeof Skills.SHOOT1
      | typeof Skills.SHOOT2
      | typeof Skills.SHOOT3,
  ) {
    offsets =
      skill_data === Skills.SHOOT1
        ? bullets_offsets_1
        : skill_data === Skills.SHOOT2
        ? bullets_offsets_2
        : bullets_offsets_3
    ...
    for (let offset of offsets) {
      ...
      if (skill_data === Skills.SHOOT2) bullet_i.damage_scale = 0.6
      else if (skill_data === Skills.SHOOT3) bullet_i.damage_scale = 0.4
  ...
  update(delta: number) {
    ...
    for (em_i = 0; em_i < this.emoticons.length; em_i++) {
      if (this.emoticons[em_i].expired === true) {
        ...
        em_i--
      } else this.emoticons[em_i].update(delta)
    }

    if (this.emitter) this.emitter.update(delta)
    if (this.poisoning_emitter) {
      this.poisoning_emitter.update(delta)
      if (this.poisoning_emitter.expired === true) this.poisoning_emitter = null
    }
  }
