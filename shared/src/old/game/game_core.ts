...
export default class GameCore extends GameMap {
    ...
    for (let i = 0; i < init_data.length; i++) {
      ...
      player
        .setPos(
          ...
          (Math.sin(a) * PARAMS.spawn_radius) / 2,
        )
        .setRot(-a + Math.PI / 2)
  ...
  respawnPlayer(player: Player) {
    ...
    this.last_respawn_angle +=
      (Math.PI * 2.0) / this.players.length + (Math.PI * 2.0) / 16
    player
      .setPos(
        (Math.cos(a) * PARAMS.spawn_radius) / 2,
        (Math.sin(a) * PARAMS.spawn_radius) / 2,
  ...
  spawnEnemy(class_index: number) {
    //returns reference to created Enemy instance (server-side use)
    //@ts-ignore
  ...
  drawDeathMark(x: number, y: number, color: ColorI) {
    //death symbol after player death
    this.color = color.hex

    super.drawLine(
      x - PARAMS.death_mark_size,
      y - PARAMS.death_mark_size,
      x + PARAMS.death_mark_size,
      y + PARAMS.death_mark_size,
      0.01,
    )
  ...
  drawStain(stain_index: number, x: number, y: number, size: number) {
    //poison stain
  ...
  static getRandomEnemyClassIndex() {
    ...
    for (let i = 0; i < ENEMY_SPAWN_PROBABILITIES.length; i++) {
      if (random_value < ENEMY_SPAWN_PROBABILITIES[i] + prop_sum) return i
