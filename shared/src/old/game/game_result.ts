...
function calculateCoinsReward(
  ...
  if (players_count < 2) return 0

  let beaten_players = players_count - (position + 1)
  return Math.round(
    beaten_players * COINS_PER_BEATEN_PLAYER +
      (points / GameCore.GET_PARAMS().points_for_enemy_kill) *
        COINS_FOR_POINTS_BONUS_FACTOR *
        ((players_count - 1) / 8),
  )
}
...
function eloRating(Ra: number, Rb: number, d: boolean) {
  ...
  if (d) return eloK * (1 - Pa)
  // Case -2 When Player B wins
  else return eloK * (0 - Pa)
}
...
function calculateRankReward(players: Player[], target_index: number) {
  ...
  if (!players[target_index].account_id) return 0 //ignore guests (update 07.07.2019)

  for (let i = 0; i < players.length; i++) {
    let Rb: number = players[i].rank

    if (i !== target_index) total_reward += eloRating(Ra, Rb, i > target_index)
  }
