...
export default class Map extends PaintLayer {
  ...
  constructor() {
    ...
    this.updatables = [
      //contains array that contains objects with update(delta) method
      this.players,
      this.enemies,
      this.enemy_spawners,
      this.items,
    ...
    for (ui = 0; ui < this.updatables.length; ui++) {
      ...
      for (obj_i = 0; obj_i < temp_arr.length; obj_i++) {
        if (temp_arr[obj_i].expired) {
          ...
          temp_arr[obj_i].update(
            (timestamp - temp_arr[obj_i].timestamp) / 1000.0,
          )
          temp_arr[obj_i].timestamp = 0
        } //object timestamp === 0
        else temp_arr[obj_i].update(delta)
      }
    }
  }

  loadMap(map: MapJSON_I, res = PAINTER_RESOLUTION.LOW) {
    //synchronous
    try {
