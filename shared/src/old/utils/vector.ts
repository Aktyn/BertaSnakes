...
class Vector {
  ...
  dot(in_vec: Vector) {
    //@in_vec - vector instance of same type and size
    values_sum = 0
  ...
  scaleBy(factor: number) {
    for (it = 0; it < this._vec_size; it++) this._buffer[it] *= factor
  }

  //STATIC METHOD FOR CALCULATIONS

  //returns squared distance between two 2D points
  static distanceSqrt(p1: Point2D_I, p2: Point2D_I) {
    //@p1 and p2 - objects with x, and y members
