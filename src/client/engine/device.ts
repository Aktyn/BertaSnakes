namespace Device {
	...
	function onFullscrenChange(e: Event) {
		refreshInfo(info);
	}

	if (document.addEventListener) {
	    document.addEventListener('webkitfullscreenchange', onFullscrenChange, false);
	    document.addEventListener('mozfullscreenchange', onFullscrenChange, false);
	    document.addEventListener('fullscreenchange', onFullscrenChange, false);
	...
	export function setOrientation(target: number) {//works only in fullscreen mode
		...
		screen.orientation.lock(target_name).catch((e: Error) => {
			...
			// document.writeln('doesnt work ' + e.message);

			try {
				//@ts-ignore
				screen.msLockOrientation.lock(target_name);
				//@ts-ignore
				screen.mozLockOrientation.lock(target_name);
			}
			catch(e) {}
		});
	}
