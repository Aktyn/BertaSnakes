...
namespace COMMON {
	...
	export function createLoader(color = '#f44336') {
	...
	export function createSwitcher(onSwitch?: (state: boolean) => void) {
	...
	export function createNumberInput(min: number, max: number, prefix = '', postfix = '') {
	...
	export function createUserEntry(user: UserInfo) {
	...
	export function trimString(str: string, max_len = 10, trimmer = '...') {
