...
const RoomView = (function() {
	...
	function createMapPreviewWidget(map_name: string) {
	...
	function createClockWidget(minutes: number) {
	...
	return class RoomView extends SessionWidget {
		private added_users_entries: UserEntry[] = [];
		...
		openRoomSettings() {
			...
			var duration_input = 
				COMMON.createNumberInput(MINIMUM_MINUTES, MAXIMUM_MINUTES, undefined, ' min')
			...
			var applySettings = () => {////////////////////////////////////////////////////
				...
				Network.sendRoomUpdateRequest(name_input.value, sits_input.getValue(),
					...
					this.roomWidget.getChildren('.map_preview.selected > label').innerHTML,
