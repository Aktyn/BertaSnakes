const ServerGame = (function() {
	...
	const GameCore = require('./../include/game/game_core.js');
	const Colors = require('./../include/game/common/colors.js');
	const Vector = require('./../include/utils/vector.js');
	const Item = require('./../include/game/objects/item.js');
	const PoisonousEnemy = require('./../include/game/objects/poisonous_enemy.js');
	const Bullet = require('./../include/game/objects/bullet.js');
	const Bomb = require('./../include/game/objects/bomb.js');
	const Skills = require('./../include/game/common/skills.js');
	const Effects = require('./../include/game/common/effects.js');
	const GameResult = require('./../include/game/game_result.js');
	...
	var wave_i, chunk_it, chunk_ref, ss_i, obj_i, p_i, e_i, s_i, b_i, e_h, e_h2, s_h, 
		async_p_i, p_it, async_p_it, async_s, r_p_i,
	...
	return class extends GameCore {
		constructor(map, room) {
			...
			if(room.gamemode === RoomInfo.GAME_MODES.COOPERATION)
				this.maximum_enemies = MAXIMUM_ENEMIES;
			else if(room.gamemode === RoomInfo.GAME_MODES.COMPETITION)
		...
		onClientMessage(client_id, data) {//@data - Uint8Array
		...
		bouncePainter(object, color, bounce_vector) {//NOTE - this.bounceOutOfColor, not super !!!
		...
		onPlayerPainterCollision(player, color) {
			...
			if(Colors.compareByteBuffers(Colors.WALLS.byte_buffer, color) === true) {
				...
				if(player.effects.isActive( Effects.POISONING ) === false) {
					player.effects.active( Effects.POISONING );
		...
		onPlayerEnemyCollision(player, enemy) {
			...
			if( player.effects.isActive(Effects.SHIELD) === false && 
				player.effects.isActive(Effects.SPAWN_IMMUNITY) === false ) 
			{
		...
		onPlayerItemCollision(player, item) {
			...
			switch(item.type) {
				...
				case Item.TYPES.SPEED:
					player.effects.active( Effects.SPEED );
					break;
			}
		...
		applySkillEffect(player, skill, player_i, skill_i, immediately_response) {
			...
			switch(skill.data) {
				...
				case Skills.BOUNCE_SHOT:
					...
					if(player.effects.isActive(Effects.POISONING)) {
						skill.stopUsing();
					...
					for(e_i=0; e_i<this.enemies.length; e_i++) {
						...
						Vector.distanceSqrt(this.enemies[e_i], player) <= radius_pow ) {
							this.onPlayerAttackedEnemy(
								player, this.enemies[e_i], GameCore.PARAMS.energy_blast_damage);
						}
					}
						...
						for(p_i=0; p_i<this.players.length; p_i++) {
							...
							Vector.distanceSqrt(this.players[p_i], player) <= radius_pow ) {
								this.onPlayerAttackedPlayer(
									player, this.players[p_i], GameCore.PARAMS.energy_blast_damage);
							}
						}
					}
					break;
				case Skills.SHIELD:
					player.effects.active( Effects.SHIELD );
			...
			for(r_p_i=0; r_p_i<this.respawning_players.length; r_p_i++) {
				if( (this.respawning_players[r_p_i].time -= delta) <= 0) {//spawning finished
					...
					this.dataForClients.push(NetworkCodes.ON_IMMUNITY_EFFECT, 
						this.players.indexOf(p_it));
					p_it.effects.active( Effects.SPAWN_IMMUNITY );
		...
		update(delta) {//NOTE - server side delta is fixed
			delta /= 1000.0;
