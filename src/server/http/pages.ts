...
const topbarHTML = topbar.getHTML();
...
function createAndSerialize(options: Options) {
	options = options || {};

	const dom = new JSDOM(`<!DOCTYPE html><html lang="en-US"></html>`);
	//@ts-ignore
	const document: any = dom.window.document;//document in js version
...
export default {//PAGES
	...
	homepage: createAndSerialize({modify: (document: Document) => {
	...
	forum: createAndSerialize({title: 'Berta Snakes Forum', modify: (document: Document) => {
	...
	info: createAndSerialize({modify: (document: Document) => {
	...
	gallery: createAndSerialize({modify: (document: Document) => {
	...
	ranking: createAndSerialize({modify: (document: Document) => {
		document.head.appendChild( createScriptNode(document, 'webjs/ranking.js', true) );
	}}),
	user: createAndSerialize({modify: (document: Document) => {
	...
	game: createAndSerialize({modify: (document: Document) => {
	...
	account: createAndSerialize({modify: (document: Document) => {
	...
	login: createAndSerialize({modify: (document: Document) => {
