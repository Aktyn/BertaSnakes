...
function createVisitsTable(VISITS: VisitJSON[]) {
	let visits_table = $$.create('TABLE').addClass('dark_evens').setStyle({'width': '100%'}).addChild(
		$$.create('TR')
...
function refreshStatistics() : void {
	...
	$$.postRequest('statistics_request', {
		...
		$$('#visits_table_container').html('').addChild( 
			$$.create('DIV').addChild( createVisitsTable(res.VISITS) ).setStyle({
				'max-height': '500px',
				'overflow-y': 'auto'
			}) 
		);
