...
(function GameOfLifeInit() {
	...
	const MAP = (function(ctx) {
		...
		return {
			...
			set: (data: Uint8ClampedArray, x: number, y: number, active: boolean) => {
				p_i = (x + y * width)*4;
				if(active) {
			...
			get: (data: Uint8ClampedArray, x: number, y: number) => {
				p_i = (x + y * width)*4;
				for(p_ii=0; p_ii<4; p_ii++) {
	...
	})( <CanvasRenderingContext2D>canvas.getContext('2d', {antialias: false, alpha: true}) );
	...
	const logic = {
		...
		//@ts-ignore
		countNeighbours: function(data: Uint8ClampedArray, x: number, y: number) {
			nb = 0;
			for(xx=-1; xx<=1; xx++) {
				for(yy=-1; yy<=1; yy++) {
		...
		step: function(current_state: Uint8ClampedArray, data: Uint8ClampedArray) {
			for(y=0; y<height; y++) {
				for(x=0; x<width; x++) {
