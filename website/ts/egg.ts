...
(function(w, h) {
	...
	function getScriptPath(foo: any) {
		return window.URL.createObjectURL(
			new Blob([foo.toString().match(/^\s*function\s*\(\s*\)\s*\{(([\s\S](?!\}$))*[\s\S])/)[1]],
				{type:'text/javascript'})); 
	}

	function drawFractal(input_values: object): Worker {
		var calc_thread = new Worker(getScriptPath(function() {
		    self.addEventListener('message', function(e) {
		    	var linear_interpolate = function(c1: number[], c2: number[], factor: number) {
		    		if(c1 === undefined || c2 === undefined) return [0, 0, 0];
		    		return c1.map((v, i) => {return v*(1-factor)+c2[i]*factor;});
		    	};

		    	//@ts-ignore
		    	function fix255(v: number, t: number, p: number): number[] { 
		    		return Array.from(arguments).map(x => ~~(x*255));
		    	}
	...
	var draw_thread: Worker | undefined;
	let params = {
		x: -0.77568377, 
		y: 0.13646737, 
		scale: 1,
	...
	//@ts-ignore
	c.onwheel = c.onscroll = function(e: WheelEvent) {
	...
	let drag: DragP | undefined, last_drag: DragP | undefined;
	var setDrag = (e: MouseEvent) => drag = {x: e.clientX, y: e.clientY};
