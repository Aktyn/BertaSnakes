...
(function() {
	...
	function loadGameInfo(id: number | string) {
		$$('#game_details').addChild( body_loader = COMMON.createLoader() );

		$$.postRequest('/game_info', {id: id}, raw_res => {
			if(body_loader)
				body_loader.remove();
			...
			var result_rows: $_face[] = [];

			try {
				let results: {players_results: PlayerResult[]} = JSON.parse( res.RESULT );
				results.players_results.forEach((p_r, index: number) => {//for each player's result
					result_rows.push(
						$$.create('TR')
			...
			$$('#game_details').addChild(
				...
				$$.create('DIV').setStyle({'margin': '0px'}).addChild(
					$$.create('TABLE').setStyle({'width': '100%'}).addChild(//game info
						$$.create('TR')
						...
						$$.create('TR')
							...
							.addChild( $$.create('TD').setText(res.TIME) )
					)
				).addChild(
					$$.create('TABLE').setStyle({'width': '100%'}).addClass('dark_evens').addChild(
						$$.create('TR').addChild( 
